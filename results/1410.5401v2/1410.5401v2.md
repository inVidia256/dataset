...
The text content from the image is "Neural Turing Machines".  
...
In neuroscience, the working memory process has been ascribed to the functioning of a system composed of the prefrontal cortex and basal ganglia (Goldman-Rakic, 1995).  
...
"s to our work include differentiable models of attention (Graves"  
...
\[
0 \leq w_t(i) \leq 1 \quad \text{for all } i = 1, 2, \ldots, N.
\]  
...
\[
\mathbf{r}_t = \sum_{i=1}^{M} \alpha_i \mathbf{M}_t(i)
\]

where \( \alpha_i \) are the coefficients of the convex combination, and they satisfy the conditions:
\[
0 \leq \alpha_i \leq 1 \quad \text{and} \quad \sum_{i=1}^{M} \alpha_i = 1.
\]  
...
$$
\tilde { \bf M } _ { t } ( i ) \leftarrow { \bf M } _ { t - 1 } ( i ) \left[ { \bf 1 } - w _ { t } ( i ) { \bf e } _ { t } \right] ,
$$  
...
$$
\sum _ { i } w _ { t } ( i ) = 1 , \qquad 0 \leq w _ { t } ( i ) \leq 1 , \, \forall i .
$$  

The text content from the image is "3.1 Reading".  

$$
{ \bf r } _ { t } \leftarrow \sum _ { i } w _ { t } ( i ) { \bf M } _ { t } ( i ) ,
$$  

The text content from the image is "3.2 Writing".  

The text in the image reads: "ferentiable with respect to both the memory and the weighting"  
...
The text content from the image is "3.3 Addressing Mechanisms".  
...
After interpolation, each head emits a shift weighting s_t that defines a normalised distribution over the allowed integer shifts. For example, if shifts between -1 and 1 are allowed, s_t has three elements corresponding to the degree to which shifts of -1, 0 and 1 are performed. The simplest way to define the shift weightings is to use a softmax layer of the appropriate size attached to the controller. We also experimented with another technique, where the controller emits a single scalar that is interpreted as the lower bound of a width one uniform distribution over shifts. For example, if the shift scalar is 6.7, then s_t(6) = 0.3, s_t(7) = 0.7, and the rest of s_t is zero.  
...
$$
{ \bf w } _ { t } ^ { g } \leftarrow g _ { t } { \bf w } _ { t } ^ { c } + ( 1 - g _ { t } ) { \bf w } _ { t - 1 } .
$$  
...
$$
\tilde { w } _ { t } ( i ) \leftarrow \sum _ { j = 0 } ^ { N - 1 } w _ { t } ^ { g } ( j ) \, s _ { t } ( i - j )
$$  

$$
w _ { t } ( i ) \leftarrow \frac { \tilde { w } _ { t } ( i ) ^ { \gamma _ { t } } } { \sum _ { j } \tilde { w } _ { t } ( j ) ^ { \gamma _ { t } } }
$$  
...
iterative difference in the way the two models solve the problem  
...
During training, we used a minimum of 2 items and a maximum of 6 items in a single episode.  
...
"10: Associative Recall Learning Curves for NTM and LSTM"  
...
where c is the five bit previous context, B is the value of the next bit and N₀ and N₁ are respectively the number of zeros and ones observed after c so far in the sequence. We can therefore compare NTM to the optimal predictor as well as LSTM. To assess performance we used a validation set of 1000 length 200 sequences sampled from the same distribution as the training data. As shown in Figure 13, NTM achieves a small, but significant performance advantage over LSTM, but never quite reaches the optimum cost.  
...
Each input sequence contained 20 binary vectors with corresponding priorities, and each target sequence was the 16 highest-priority vectors in the input.  
...
The text content from the image is "4.5 Priority Sort".  
...
1: NTM with Feedforward Controller Experimental Settings  
...
The text content from the image is "References".  
...
Hadley, K. F. (2009). The problem of rapid variable creation. Neural computation, 21(2):510–532.
