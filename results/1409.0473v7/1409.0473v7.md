...
$$
p ( \mathbf { y } ) = \prod _ { t = 1 } ^ { T } p ( y _ { t } \mid \{ y _ { 1 } , \cdots , y _ { t - 1 } \} , c ) ,
$$  
...
Figure 1: The graphical illustration of the proposed model trying to generate the t-th target word y_t given a source sentence (x_1, x_2, . . . , x_T).  
...
$$
p ( y _ { t } \mid \{ y _ { 1 } , \cdots , y _ { t - 1 } \} , c ) = g ( y _ { t - 1 } , s _ { t } , c ) ,
$$  

$$
p ( y _ { i } | y _ { 1 } , \ldots , y _ { i - 1 } , \mathbf { x } ) = g ( y _ { i - 1 } , s _ { i } , c _ { i } ) ,
$$  
...
$$
s _ { i } = f \bigl ( s _ { i - 1 } , y _ { i - 1 } , c _ { i } \bigr ) .
$$  

The text content from the image is:

"where y = (y₁, ⋯ , yₜᵧ). With an RNN, each conditional probability is modeled as"  
...
The text content from the image is "5 RESULTS".  
...
The text content from the image is "5.2.1 ALIGNMENT".  
...
The text content from the image is "6.1 LEARNING TO ALIGN".  
...
Cho, K., van Merrienboer, B., Gulcehre, C., Bougares, F., Schwenk, H., and Bengio, Y. (2014a). Learning phrase representations using RNN encoder-decoder for statistical machine translation. In Proceedings of the Empirical Methods in Natural Language Processing (EMNLP 2014). to appear.
...
Schwenk, H. (2012). Continuous space translation models for phrase-based statistical machine translation. In M. Kay and C. Boitet, editors, Proceedings of the 24th International Conference on Computational Linguistics (COLING), pages 1071–1080. Indian Institute of Technology Bombay.  
...
\`\`\`
where e(y_{i-1}) ∈ ℝ^m is an m-dimensional embedding of a word y_{i-1}, and r_i is the output of the reset gates (see below). When y_i is represented as a 1-of-K vector, e(y_i) is simply a column of an embedding matrix E ∈ ℝ^{m×K}. Whenever possible, we omit bias terms to make the equations less cluttered.
\`\`\`  
...
"where ⊙ is an element-wise multiplication, and \( z_i \) is the output of the update gates (see below). The proposed updated state \( \tilde{s}_i \) is computed by"  
...
$$
\begin{array} { l } { { z _ { i } = \sigma \left( W _ { z } e ( y _ { i - 1 } ) + U _ { z } s _ { i - 1 } + C _ { z } c _ { i } \right) , } } \\ { { r _ { i } = \sigma \left( W _ { r } e ( y _ { i - 1 } ) + U _ { r } s _ { i - 1 } + C _ { r } c _ { i } \right) , } } \end{array}
$$  
...
"E is the word embedding matrix for the target language. W, Wz, Wr ∈ ℝn×m, U, Uz, Ur ∈ ℝn×n, and C, Cz, Cr ∈ ℝn×2n are weights. Again, m and n are the word embedding dimensionality and the number of hidden units, respectively. The initial hidden state s0 is computed by s0 = tanh (Ws h̅1), where Ws ∈ ℝn×n."  

The text content from the image is as follows:

\`\`\`
\overline{E} \in \mathbb{R}^{m \times K_x} is the word embedding matrix.
\overrightarrow{W}, \overrightarrow{W_z}, \overrightarrow{W_r} \in \mathbb{R}^{n \times m},
\overrightarrow{U}, \overrightarrow{U_z}, \overrightarrow{U_r} \in \mathbb{R}^{n \times n}
are weight matrices.
m and n are the word embedding dimensionality and the number of hidden units, respectively.
...
We concatenate the forward and backward states to obtain the annotations \( (h_1, h_2, \cdots, h_{T_x}) \), where  

$$
```latex
\begin{align*}
\overrightarrow{h}_i &= \tanh \left( \overrightarrow{W} \overline{E} x_i + \overrightarrow{U} \left[ \overrightarrow{r'}_i \circ \overrightarrow{h}_{i-1} \right] \right) \\
...
$$
```latex
\tilde{s}_i = \tanh \left( W_E y_{i-1} + U [r_i \circ s_{i-1}] + C c_i \right)
...
\overrightarrow{h}_i = 
\begin{cases} 
(1 - \overrightarrow{z}_i) \circ \overrightarrow{h}_{i-1} + \overrightarrow{z}_i \circ \underline{\overline{h}}_i & \text{, if } i > 0 \\
0 & \text{, if } i = 0
\end{cases}
```
$$  
...
$$
\mathbf { y } = ( y _ { 1 } , \ldots , y _ { T _ { y } } ) , \; y _ { i } \in \mathbb { R } ^ { K _ { y } } ,
$$  

The backward states (h̅₁, …, h̅ₜₓ) are computed similarly. We share the word embedding matrix E̅ between the forward and backward RNNs, unlike the weight matrices.  
...
The text content from the image is "A.2.1 ENCODER".  
...
"ate \( s_i \) of the decoder given the annotations from the encoder is computed b"

It appears to be incomplete, as it cuts off at the end with "b".  
...
"W_o ∈ ℝ^K_y×l, U_o ∈ ℝ^2l×n, V_o ∈ ℝ^2l×m and C_o ∈ ℝ^2l×2n are weight matrices. This can be understood as having a deep output (Pascanu et al., 2014) with a single maxout hidden layer (Goodfellow et al., 2013)."  
...
$$
t _ { i } = \left[ \operatorname* { m a x } \left\{ \tilde { t } _ { i , 2 j - 1 } , \tilde { t } _ { i , 2 j } \right\} \right] _ { j = 1 , \ldots , l } ^ { \top }
$$  
...
The text content from the image is "A.2.3 MODEL SIZE".  
...
sent the statistics related to training all the models used in the experiments.  
