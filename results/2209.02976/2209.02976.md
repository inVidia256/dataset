...
Figure 1: Comparison of state-of-the-art efficient object detectors. Both latency and throughput (at a batch size of 32) are given for a handy reference. All models are tested with TensorRT 7 except that the quantized model is with TensorRT 8.  
...
- **Network Design:**
  - **Backbone:** Compared with other mainstream architectures, we find that RepVGG [3] backbones are equipped with more feature representation power in small networks at a similar inference speed, whereas it can hardly be scaled to obtain larger models due to the explosive growth of the parameters and computational costs. In this regard, we take RepBlock [3] as the building block of our small networks. For large models, we revise a more efficient CSP [43] block, named CSPStackRep Block.
  - **Neck:** The neck of YOLOv6 adopts PAN topology [24] following YOLOv4 and YOLOv5. We enhance the neck with RepBlocks or CSPStackRep Blocks to have RepPAN.
  - **Head:** We simplify the decoupled head to make it more efficient, called Efficient Decoupled Head.  

The loss functions of the mainstream anchor-free object detectors contain classification loss,  
...
The text content from the image is "2.1. Network Design".  

The text content from the image is "2.1.1 Backbone".  
...
and S are shown). Note for M/L, RepBlocks is replaced with CSPStackRe  
...
The detection head of YOLOv5 is a coupled head with parameters shared between the classification and localization branches, while its  
...
The text content from the image is "2.2. Label Assignment".  

2.1.2 Neck  

The text content from the image is "2.1.3 Head".  
...
The text content from the image is "2.3.1 Classification Loss".  

The text content from the image is "2.3. Loss Functions".  

2.4.1 More training epochs  

The text content from the image is "2.3.2 Box Regression Loss".  

The text content from the image is "2.3.3 Object Loss".  

The text content from the image is "2.4.2 Self-distillation".  
...
$$
L _ { K D } = K L ( p _ { t } ^ { c l s } \| p _ { s } ^ { c l s } ) + K L ( p _ { t } ^ { r e g } \| p _ { s } ^ { r e g } ) ,
$$  
...
$$
L _ { t o t a l } = L _ { d e t } + \alpha L _ { K D } ,
$$  
...
The text content from the image is "3.3.1 Network".  

The text content from the image is "3.3. Ablation Study".  
...
"Table 3: Ablation study on the neck settings of YOLOv6-L.
SiLU is selected as the activation function."  

$$
L _ { d e t } = L _ { c l s } + \lambda L _ { r e g } + \mu L _ { o b j } ,
$$  

The text content from the image is "3.3.3 Loss functions".  

The text content from the image is "3.3.2 Label Assignment".  
...
The text content from the image is "3.5. Quantization Results".  

3.5.2 QAT  

The text content from the image is "3.5.1 PTQ".  
...
The text in the image reads: "400 epochs is a better convergence scheme for YOLOv6."  
...
\`\`\`plaintext
[1] He, K., Zhang, X., Ren, S., & Sun, J. (2016). Deep residual learning for image recognition. In Proceedings of the IEEE conference on computer vision and pattern recognition, pages 770–778.

[2] He, K., Zhang, X., Ren, S., & Sun, J. (2016). Identity mappings in deep residual networks. In European conference on computer vision, pages 630–645. Springer.

[3] Huang, G., Liu, Z., Van Der Maaten, L., & Weinberger, K. Q. (2017). Densely connected convolutional networks. In Proceedings of the IEEE conference on computer vision and pattern recognition, pages 4700–4708.
...
[5] He, J., Erfani, S., Ma, X., Bailey, J., Chi, Y., & Hua, X.-S. (2021). α-iou: A family of power intersection over union losses for bounding box regression. Advances in Neural Information Processing Systems, 34:20230–20242.

[6] Ge, Z., Liu, S., Li, F., Wang, Z., Li, Z., & Sun, J. (2021). Yolox: Exceeding yolo series in 2021. arXiv preprint arXiv:2107.08430.

[7] Ge, Z., Liu, S., Li, F., Wang, Z., Li, Z., & Sun, J. (2021). Ota: Optimal transport assignment for object detection. In 2021 IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR), pages 303–312.

[8] Elfwing, S., Uchibe, E., & Doya, K. (2018). Sigmoid-weighted linear units for neural network function approximation in reinforcement learning. Neural Networks, 107:3–11.

[9] Ghiassi, G., Lin, T.-Y., & Le, Q. V. (2019). Nas-fpn: Learning scalable feature pyramid architecture for object detection. In Proceedings of the IEEE/CVF conference on computer vision and pattern recognition, pages 7036–7045.

[10] Glenn, J. (2022). YOLOv5 release v6.1. https://github.com/ultralytics/yolov5/releases/tag/v6.1.

[11] Law, H., & Deng, J. (2018). Cornernet: Detecting objects as paired keypoints. In Proceedings of the European conference on computer vision (ECCV), pages 734–750.
\`\`\`

This list contains references to various papers and articles related to computer vision and pattern recognition, including topics such as deep learning architectures, object detection, and loss functions.  
...
[11] Jiabo He, Sarah Erfani, Xingjun Ma, James Bailey, Ying Chi, and Xian-Sheng Hua. α-iou: A family of power intersection over union losses for bounding box regression. Advances in Neural Information Processing Systems, 34:20230–20242, 2021.  
...
\`\`\`markdown
In Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR), pages 9387–9396, June 2022. 2, 4, 9
...
[36] Changyong Shu, Yifan Liu, Jianfei Gao, Zheng Yan, and Chunhua Shen. Channel-wise knowledge distillation for dense prediction. In Proceedings of the IEEE/CVF International Conference on Computer Vision, pages 5311–5320, 2021.
...
[41] Zhi Tian, Chunhua Shen, Hao Chen, and Tong He. FCOS: Fully convolutional one-stage object detection. In Proc. Int. Conf. Computer Vision (ICCV), 2019.
...
[44] Shangliang Xu, Xinxin Wang, Wenyu Lv, Qinyao Chang, Cheng Cui, Kaipeng Deng, Guanzhong Wang, Qingqing Dang, Shengyu Wei, Yuning Du, et al. Pp-yoloe: An evolved version of yolo. arXiv preprint arXiv:2203.16250, 2022.  
...
[19] Xiang Li, Wenhai Wang, Xiaolin Hu, Jun Li, Jinhui Tang, and Jian Yang. Generalized focal loss v2: Learning reliable localization quality estimation for dense object detection. In Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition, pages 11632–11641, 2021.  

[45] Shangliang Xu, Xinxin Wang, Wenyu Lv, Qinyao Chang, Cheng Cui, Kaipeng Deng, Guanzhong Wang, Qingqing Dang, Shengyu Wei, Yuning Du, et al. Pp-yoloe: An evolved version of yolo. arXiv preprint arXiv:2203.16250, 2022.

[46] Ze Yang, Shaohui Liu, Han Hu, Liwei Wang, and Stephen Lin. Reppoints: Point set representation for object detection. In Proceedings of the IEEE/CVF International Conference on Computer Vision, pages 9657–9666, 2019.

[47] Jiahui Yu, Yuning Jiang, Zhangyang Wang, Zhimin Cao, and Thomas Huang. Unitbox: An advanced object detection network. In Proceedings of the 24th ACM international conference on Multimedia, pages 516–520, 2016.

[48] Mohsen Zand, Ali Etemad, and Michael A. Greenspan. Objectbox: From centers to boxes for anchor-free object detection. ArXiv, abs/2207.06985, 2022.
...
[50] Haoyang Zhang, Ying Wang, Feras Dayoub, and Niko Sunderhauf. Varifocalnet: An iou-aware dense object detector. In Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition, pages 8514–8523, 2021.

[51] Shifeng Zhang, Cheng Chi, Yongqiang Yao, Zhen Lei, and Stan Z. Li. Bridging the gap between anchor-based and anchor-free detection via adaptive training sample selection. In CVPR, 2020.
...
[53] Xingyi Zhou, Dequan Wang, and Philipp Krähenbühl. Objects as points. arXiv preprint arXiv:1904.07850, 2019.  
...
The text content from the image is "A.4. CPU Latency".  
...
"Examples of better optimized layers in YOLOv6s that are otherwise hard to quantify."  