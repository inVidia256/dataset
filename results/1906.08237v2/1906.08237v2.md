...
The text content from the image is "2.1 Background".  

The text content from the image is "2 Proposed Method".  
...
pretraining objective, XLNet improves architectural designs for pretraining  
...
To formalize the idea, let \( \mathcal{Z}_T \) be the set of all possible permutations of the length-\( T \) index sequence [1, 2, ..., T]. We use \( z_t \) and \( \mathbf{z}_{<t} \) to denote the \( t \)-th element and the first \( t-1 \) elements of a permutation \( \mathbf{z} \in \mathcal{Z}_T \). Then, our proposed permutation language modeling objective can be expressed as follows:  
...
- Independence Assumption: As emphasized by the ≈ sign in Eq. (2), BERT factorizes the joint conditional probability p(̅x | ̂x) based on an independence assumption that all masked tokens ̅x are separately reconstructed. In comparison, the AR language modeling objective (1) factorizes pθ(x) using the product rule that holds universally without such an independence assumption.  
...
- Independence Assumption: As emphasized by the ≈ sign in Eq. (2), BERT factorizes the joint conditional probability p(̃x | ̃x̂) based on an independence assumption that all masked tokens ̃x̂ are separately reconstructed. In comparison, the AR language modeling objective (1) factorizes pθ(x) using the product rule that holds universally without such an independence assumption.
- Input noise: The input to BERT contains artificial symbols like [MASK] that never occur in downstream tasks, which creates a pretrain-finetune discrepancy. Replacing [MASK] with original tokens as in [10] does not solve the problem because original tokens can be only used with a small probability — otherwise Eq. (2) will be trivial to optimize. In comparison, AR language modeling does not rely on any input corruption and does not suffer from this issue.
...
$$
p _ { \theta } ( X _ { z _ { t } } = x \mid \mathbf { x } _ { z < t } ) = { \frac { \exp \left( e ( x ) ^ { \top } g _ { \theta } ( \mathbf { x } _ { \mathbf { z } < t } , z _ { t } ) \right) } { \sum _ { x ^ { \prime } } \exp \left( e ( x ^ { \prime } ) ^ { \top } g _ { \theta } ( \mathbf { x } _ { \mathbf { z } < t } , z _ { t } ) \right) } } ,
$$  

The text content from the image is as follows:

"Computationally, the first layer query stream is initialized with a trainable vector, i.e., \( g_i^{(0)} = w \), while the content stream is set to the corresponding word embedding, i.e., \( h_i^{(0)} = e(x_i) \). For each self-attention layer \( m = 1, \ldots, M \), the two streams of representations are schematically2 updated."  

- The content representation \( h_\theta(\mathbf{x}_{\mathbf{z} \leq t}) \), or abbreviated as \( h_{zt} \), which serves a similar role to the standard hidden states in Transformer. This representation encodes both the context and \( x_{zt} \) itself.
- The query representation \( g_\theta(\mathbf{x}_{\mathbf{z} < t}, z_t) \), or abbreviated as \( g_{zt} \), which only has access to the contextual information \( \mathbf{x}_{\mathbf{z} < t} \) and the position \( z_t \), but not the content \( x_{zt} \), as discussed above.  
...
$$
```latex
h_{z_t}^{(m)} \leftarrow \text{Attention}(Q = h_{z_t}^{(m-1)}, KV = [\tilde{\mathbf{h}}_{(m-1)}, \mathbf{h}_{\mathbf{z} \leq t}^{(m-1)}]; \theta)
```
$$  

The image contains two equations related to attention mechanisms in a neural network context:
...
These equations describe how the query and content streams update their representations \(g_{z_t}^{(m)}\) and \(h_{z_t}^{(m)}\) respectively, using an attention mechanism with parameters \(\theta\). The query stream uses only the current token \(z_t\) as the query and does not have access to the future tokens \(x_{z_t}\), while the content stream uses both the current token \(z_t\) and its corresponding embedding \(x_{z_t}\) as part of the key-value pairs.  

The text content from the image is:

"1 a shared set of parameters as follows (illustrated in Figures 1 (a) and (b)):  
...
The text content from the image is "2.6 Discussion".  
...
more formal analysis and further discussion, please refer to Appendix A.5  

$$
```latex
\mathcal{I}_{BERT} = \log p(\text{New} \mid \text{is a city}) + \log p(\text{York} \mid \text{is a city}),
```
$$  
...
The text content from the image is "3.4 Ablation Study".  
...
importance of using Transformer-XL as the backbone neural architecture.  
...
\`\`\`
1 state-of-the-art error rates on the test sets of several text classification datasets. A
ire obtained with a 24-layer architecture with similar model sizes (aka BERT-Large
\`\`\`  
...
The text content from the image is "4 Conclusions".  
...
The results of BERT on RACE are taken from [38]. We run BERT on the other datasets using the official implementation and the same hyperparameter search space as XLNet. K is a hyperparameter to control the optimization difficulty (see Section 2.3).  
...
for soft-matching n-grams in ad-hoc search. In Proceedings of the eleventh ACM international conference on web search and data mining, pages 126–134. ACM, 2018.
...
"nmon Crawl. Common crawl. URL: http://http://commoncrawl.org, 2019."  
...
[28] Alec Radford, Karthik Narasimhan, Tim Salimans, and Ilya Sutskever. Improving language understanding by generative pre-training. URL https://s3-us-west-2.amazonaws.com/openai-assets/research-covers/language-unsupervised/language-understanding-paper.pdf, 2018.
...
[38] Shuiliang Zhang, Hai Zhao, Yuwei Wu, Zhuosheng Zhang, Xi Zhou, and Xiang Zhou. Dual co-matching network for multi-choice reading comprehension. arXiv preprint arXiv:1901.09381, 2019.  
...
\forall t = 1, \ldots, T:
\begin{aligned}
\hat{h}_{z_t}^{(m)} &= \text{LayerNorm}\left(h_{z_t}^{(m-1)} + \text{RelAttn}\left(h_{z_t}^{(m-1)}, \left[\tilde{\mathbf{h}}^{(m-1)}, \mathbf{h}_{\mathbf{z} \leq t}^{(m-1)}\right]\right)\right) \\
h_{z_t}^{(m)} &= \text{LayerNorm}\left(\hat{h}_{z_t}^{(m)} + \text{PosFF}\left(\hat{h}_{z_t}^{(m)}\right)\right) \\
\hat{g}_{z_t}^{(m)} &= \text{LayerNorm}\left(g_{z_t}^{(m-1)} + \text{RelAttn}\left(g_{z_t}^{(m-1)}, \left[\tilde{\mathbf{h}}^{(m-1)}, \mathbf{h}_{\mathbf{z} < t}^{(m-1)}\right]\right)\right) \\
g_{z_t}^{(m)} &= \text{LayerNorm}\left(\hat{g}_{z_t}^{(m)} + \text{PosFF}\left(\hat{g}_{z_t}^{(m)}\right)\right)
\end{aligned}
```
$$  

The text content from the image is "A.3.1 RACE Dataset".  

Here, we provide the implementation details of the two-stream attention with a Transformer-XL backbone.  

$$
p _ { \theta } ( X _ { z _ { t } } = x \mid \mathbf { x } _ { z < t } ) = \frac { \exp \left( e ( x ) ^ { \top } g _ { z _ { t } } ^ { ( M ) } \right) } { \sum _ { x ^ { \prime } } \exp \left( e ( x ^ { \prime } ) ^ { \top } g _ { z _ { t } } ^ { ( M ) } \right) } ,
$$  

A.2 Two-Stream Attention  

The text content from the image is "A.3 Datasets".  

Target-aware prediction distribution:  

$$
\underbrace { p _ { \theta } ( X _ { i } = x \mid \mathbf { x } _ { \mathbf { z } < t } ) } _ { z _ { t } ^ { ( 1 ) } = i , \; \mathbf { z } _ { < t } ^ { ( 1 ) } = \mathbf { z } _ { < t } } = \underbrace { p _ { \theta } ( X _ { j } = x \mid \mathbf { x } _ { \mathbf { z } < t } ) } _ { z _ { t } ^ { ( 1 ) } = j , \; \mathbf { z } _ { < t } ^ { ( 2 ) } = \mathbf { z } _ { < t } } = { \frac { \exp \left( e ( x ) ^ { \top } h ( \mathbf { x } _ { \mathbf { z } < t } ) \right) } { \sum _ { x ^ { \prime } } \exp \left( e ( x ^ { \prime } ) ^ { \top } h ( \mathbf { x } _ { \mathbf { z } < t } ) \right) } } .
$$  

A.3.2 SQuAD  

$$
\mathbf { z } _ { < t } ^ { ( 1 ) } = \mathbf { z } _ { < t } ^ { ( 2 ) } = \mathbf { z } _ { < t } \quad \mathrm { b u t } \quad z _ { t } ^ { ( 1 ) } = i \neq j = z _ { t } ^ { ( 2 ) } .
$$  

$$
\forall t = 1 , \ldots , T : \quad h _ { t } = e ( x _ { t } ) \quad \mathrm { a n d } \quad g _ { t } = w
$$  
...
A Concrete Example of How Standard LM Parameterization Fail  
...
the two permutations respectively into the naive parameterization, we have  

The text content from the image is:

"hed layer-m content representation (memory) from previous segment: \(\tilde{\mathbf{h}}^{(m)}\)"  

$$
\underbrace { p _ { \theta } ( X _ { i } = x \mid \mathbf { x } _ { \mathbf { z } < t } ) } _ { z _ { t } ^ { ( 1 ) } = i , \; \mathbf { z } _ { < t } ^ { ( 1 ) } = \mathbf { z } _ { < t } } = \underbrace { p _ { \theta } ( X _ { j } = x \mid \mathbf { x } _ { \mathbf { z } < t } ) } _ { z _ { t } ^ { ( 1 ) } = j , \; \mathbf { z } _ { < t } ^ { ( 2 ) } = \mathbf { z } _ { < t } } = \frac { \exp \left( e ( x ) ^ { \top } h ( \mathbf { x } _ { \mathbf { z } _ { < t } } ) \right) } { \sum _ { x ^ { \prime } } \exp \left( e ( x ^ { \prime } ) ^ { \top } h ( \mathbf { x } _ { \mathbf { z } _ { < t } } ) \right) } .
$$  
...
The text content from the image is "A.3.4 GLUE Dataset".  

The text content from the image is "A.4 Hyperparameters".  
...
To prove a general point beyond one example, we now turn to more formal expressions. Inspired by previous work [37], given a sequence x = [x₁, ⋯ , xₜ], we define a set of target-context pairs of interest, 𝒦 = {(x, 𝔰)}, where 𝔰 is a set of tokens in x that form a context of x. Intuitively, we want the model to learn the dependency of x on 𝔰 through a pretraining loss term log p(x | 𝔰). For example, given the above sentence, the pairs of interest 𝒦 could be instantiated as:  

$$
```latex
\mathcal{J}_{BERT} = \sum_{x \in \mathcal{T}} \log p(x \mid \mathcal{N}); \quad 
\mathcal{J}_{XLNet} = \sum_{x \in \mathcal{T}} \log p(x \mid \mathcal{N} \cup \mathcal{T}_{<x})
```
$$  
...
$$
{ \mathcal { I } } = \left\{ ( x = \mathrm { Y o r k } , { \mathcal { U } } = \{ \mathrm { N e w } \} ) , \, \, ( x = \mathrm { Y o r k } , { \mathcal { U } } = \{ \mathrm { c i t y } \} ) , \, \, ( x = \mathrm { Y o r k } , { \mathcal { U } } = \{ \mathrm { N e w , ~ c i t y } \} ) , \, \, \cdots \right\} .
$$  
...
1.5.3 Bridging the Gap Between Language Modeling and Pretraining  

With a deep root in density estimation [4, 32, 24], language modeling has been a rapidly-developing research area [9, 1, 3]. However, there has been a gap between language modeling and pretraining due to the lack of the capability of bidirectional context modeling, as analyzed in Section A.5.2. It has even been challenged by some machine learning practitioners whether language modeling is a meaningful pursuit if it does not directly improve downstream tasks [5]. XLNet generalizes language modeling and bridges such a gap. As a result, it further "justifies" language modeling research. Moreover, it becomes possible to leverage the rapid progress of language modeling research for pretraining. As an example, we integrate Transformer-XL into XLNet to demonstrate the usefulness of the latest language modeling progress.  
...
Figure 4: Illustration of the permutation language modeling objective for predicting \( x_{3} \) given the same input sequence \( x \) but with different factorization orders.  
