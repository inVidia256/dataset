...
This appears to be the title or description of an event related to neural information processing systems, specifically NeurIPS 2020, which took place in Vancouver, Canada.  
...
Second line of work questions if full attention is essential and have tried to come up with approaches that do not require full attention, thereby reducing the memory and computation requirements. Prominently, Dai et al. [21], Sukhbaatar et al. [82], Rae et al. [74] have proposed auto-regressive models that work well for left-to-right language modeling but suffer in tasks which require bidirectional context. Child et al. [16] proposed a sparse model that reduces the complexity to \( O(n\sqrt{n}) \), Kitaev et al. [49] further reduced the complexity to \( O(n \log(n)) \) by using LSH to compute nearest neighbors.  
...
translates to roughly being able to handle input sequences of length 512 tokens. This reduces its direct applicability to tasks that require larger context, like QA [60], document classification, etc.  

The text content from the image is "1.1 Related Work".  

To summarize, our main contributions are:  

forming attention mechanism scaling to much longer sequence lengths (8x  
...
"Let us consider the simplest random graph construction, known as Erdős-Rényi model, where each edge is independently chosen with a fixed probability. In such a random graph with just Θ(n) edges, the shortest path between any two nodes is logarithmic in the number of nodes [17, 43]. As a consequence, such a random graph approximates the complete graph spectrally and its second eigenvalue (of the adjacency matrix) is quite far from the first eigenvalue [9, 10, 6]. This property leads to a rapid mixing time for random walks in the graph, which informally suggests that information can flow fast between any pair of nodes. Thus, we propose a sparse attention where each query attends over r random number of keys i.e. A(i, ·) = 1 for r randomly chosen keys (see Fig. 1a)."  

In this section, we describe the BIGBIRD model using the generalised attention mechanism that is used in each layer of transformer operating on an input sequence \( X = (x_1, ..., x_n) \in \mathbb{R}^{n \times d} \). The generalized attention mechanism is described by a directed graph \( D \) whose vertex set is \([n] = \{1, \ldots, n\}\). The set of arcs (directed edges) represent the set of inner products that the attention mechanism will consider. Let \( N(i) \) denote the out-neighbors set of node \( i \) in \( D \), then the \( i^{\text{th}} \) output vector of the generalized attention mechanism is defined as  
...
The text content from the image is "2 BIGBIRD Architecture".  
...
The main technical challenge is computing the contextual mapping using only sparse attention mechanism. This was done in [104] using a “selective” shift operator which shifts up entries that are in a specific interval. Key to their proof was the fact that the shift, was exactly the range of the largest entry to the smallest entry.  

Step 1: Approximate \(\mathcal{F}_{CD}\) by piece-wise constant functions. Since \(f\) is a continuous function with bounded domain \([0,1)^{n\times d}\), we will approximate it with a suitable piece-wise constant function. This is accomplished by a suitable partition of the region \([0,1)\) into a grid of granularity \(\delta\) to get a discrete set \(\mathbb{G}_\delta\). Therefore, we can assume that we are dealing with a function \(f : \mathbb{G}_\delta \to \mathbb{R}^{n\times d}\), where \(d_p(f,f') \leq \frac{\epsilon}{3}\).  

The complete Transformer encoder stack is nothing but the repeated application of a single-layer encoder (with independent parameters). We denote class of such Transformer encoders stack, defined using generalized encoder (Sec. 2), by \(\mathcal{T}_{D}^{H,m,q}\) which consists of \(H\)-heads with head size \(m\) and \(q\) is the hidden layer size of the output network, and the attention layer is defined by the directed graph \(D\).  

Finally, we need to define the function class and distance measure for proving universal approximation property. Let \(\mathcal{F}_{CD}\) denote the set of continuous functions \(f : [0, 1]^{n \times d} \to \mathbb{R}^{n \times d}\) which are continuous with respect to the topology defined by \(\ell_p\) norm. Recall for any \(p \geq 1\), the \(\ell_p\) distance is \(d_p(f_1, f_2) = \left( \int \|f_1(X) - f_2(X)\|^p dX \right)^{1/p}\).  

The mechanism incurs a cost, i.e., there is no free lunch. In Sec. 3.4, we show lower bounds by exhibiting a natural task where any sufficiently sparse mechanism will require polynomially more layers.  

The star-graph \( S \) centered at 0 is the graph defined on \( \{0, \ldots, n\} \). The neighborhood of all vertices \( i \) is \( N(i) = \{0, i\} \) for \( i \in \{1, \ldots, n\} \) and \( N(0) = \{1, \ldots, n\} \).  

Theorem 1. Given \(1 < p < \infty\) and \(\epsilon > 0\), for any \(f \in \mathcal{F}_{CD}\), there exists a transformer with sparse-attention, \(g \in \mathcal{T}^{H,m,q}_D\) such that \(d_p(f,g) \leq \epsilon\) where \(D\) is any graph containing star graph \(S\).  
...
$$
( \int \| f _ { 1 } ( X ) - f _ { 2 } ( X ) \| _ { p } ^ { p } d X ) ^ { 1 / p } .
$$  
...
The image contains a mathematical task statement as follows:

"Task 1. Given \( n \) unit vectors \(\{u_1, \ldots, u_n\}\), find \( f(u_1, \ldots, u_n) \rightarrow (u_{1*}, \ldots, u_{n*}) \) where for a fixed \( j \in [n] \), we define \( j^* = \arg \max_k \|u_k - u_j\|_2^2 \)."  

The text content from the image is "3.4 Limitations".  

The text content from the image is "3.3 Turing Completeness".  
...
"Proposition 1. There exists a single layer full self-attention \( g \in \mathcal{T}^{H=1, m=2d, q=0} \) that can evaluate Task 1, i.e., \( g(u_1, ..., u_n) = [u_1^*, ..., u_n^*] \), but for any sparse-attention graph \( D \) with \( \tilde{O}(n) \) edges (i.e., inner product evaluations), would require \( \tilde{\Omega}(n^{1-o(1)}) \) layers. We give a formal proof of this fact in App. C."  

The image contains a proposition that states:

**Proposition 1.** There exists a single layer full self-attention \( g \in \mathcal{T}^{H=1, m=2d, q=0} \) that can evaluate Task 1, i.e., \( g(u_1, \ldots, u_n) = [u_1^*, \ldots, u_n^*] \), but for any sparse-attention graph \( D \) with \( \tilde{O}(n) \) edges (i.e., inner product evaluations), would require \( \tilde{\Omega}(n^{1-o(1)}) \) layers.  
...
The pretrained BIGBIRD model from above, using the training data and report F1 on test dataset. We compare our results to the previously reported best method in Tab. 6. We see that BIGBIRD achieve nearly perfect accuracy with a 5% jump from the previous best reported accuracy.  
...
[9] F. Benaych-Georges, C. Bordenave, A. Knowles, et al. Largest eigenvalues of sparse inhomogeneous erdős-rényi graphs. Annals of Probability, 47(3):1653–1676, 2019.  
...
[91] A. Vaswani, N. Shazeer, N. Parmar, J. Uszkoreit, L. Jones, A. N. Gomez, L. Kaiser, and I. Polosukhin. Attention is all you need. In Advances in neural information processing systems, pages 5998–6008, 2017.
...
[98] X. Xiao, Z.-C. Xu, W.-R. Qiu, P. Wang, H.-T. Ge, and K.-C. Chou. ipsw (21)-pseknc: A two-layer predictor for identifying promoters and their strength by hybrid features via pseudo k-tuple nucleotide composition. Genomics, 111(6):1785–1793, 2019.
...
As described in Sec. 2, an attention mechanism is parameterized by three functions: \( Q, K, V : \mathbb{R}^d \rightarrow \mathbb{R}^m \). In this paper, we assume that they are simply matrix products: \( Q(\boldsymbol{x}) = \boldsymbol{x} W_Q \), \( K(\boldsymbol{x}) = \boldsymbol{x} W_K \), and \( V(\boldsymbol{x}) = \boldsymbol{x} W_V \), where \( W_Q, W_K, W_V \in \mathbb{R}^{d \times m} \) and \( W_V \in \mathbb{R}^{d \times d} \). In reality a multi-headed attention is used, i.e. we have not only one, but \( H \)-sets of Query/Key/Value weight matrices, \( W_Q^h, W_V^h, W_K^h \) for \( h = 1, ..., H \). Thus, for a directed graph \( D \) over \([n]\), the \( i^{\text{th}} \) output vector of the generalized attention mechanism would be  

We begin by setting up some notations following Pérez et al. [72] to formally describe the complete architecture of Transformers. A single layer of Transformer encoder is a parametric function Enc receiving a sequence \( X = (x_1, ..., x_n) \) of vectors in \( \mathbb{R}^d \) and returning a sequence \( Z = (z_1, ..., z_n) \) of the same length. Each \( z_i \) is a \( d \)-dimensional vector as well. We interchangeably treat the sequence \( X \) as a matrix in \( \mathbb{R}^{n \times d} \). Enc has two components:  

In this section, we will present the full proof of theorem 1. The proof will contain three parts. The first and the third part will largely follow standard techniques. The main innovation lies in the second part.  
...
Additional Notation We introduce a few pieces of additional notation that will be useful. Let \([a, b)_\delta = \{a, a + \delta, \ldots, a + \lfloor \frac{b-a}{\delta} \rfloor \cdot \delta\}\). Therefore, \([0, 1)_\delta = \{0, \delta, 2\delta, \ldots, (1-\delta)\}\). We use \(1[\mathcal{E}]\) to denote the indicator variable; it is 1 if the event \(\mathcal{E}\) occurs and 0 otherwise.  

First, we consider a suitable partition of the region (0,1) into a grid of granularity δ, which we denote by Gδ. We do this using Lemma 8 from Yun et al. [104], which we restate for completeness:  

The text content from the image is:

"Lemma 1 (Lemma 8 [104]). For any given \( f \in \mathcal{F}_{CD} \) and \( 1 \leq p \leq \infty \), there exists a \( \delta > 0 \) such that there exists a piece-wise constant function \( \tilde{f} \) with \( d_p(f, \tilde{f}) \leq \frac{\epsilon}{3} \). Concretely, \( \tilde{f} \) is defined as"  

$$
```latex
\bar{f}(X) = \sum_{P \in \mathbb{G}_{\delta}} f(P) \cdot 1[\|ReLU(X - P)\|_\infty \leq \delta]
```
$$  

$$
```latex
\text{ATTN}_D(\mathbf{X})_i = \sum_{h=1}^{H} \sigma \left( (\mathbf{x}_i W_Q^h) (\mathbf{X}_{N(i)} W_K^h)^T \right) \cdot (\mathbf{X}_{N(i)} W_V^h)
```
$$  
...
```latex
O(a_i) = \text{ReLU}(a_i W_1 + b_1) W_2 \cdot + b_2
```
...
$$
```latex
z_i = O(\alpha_i) + \alpha_i \quad \text{where} \quad \alpha_i = \text{ATTN}(X)_i + x_i
```
$$  

A.1 Notation  

1. An attention mechanism ATTN that takes in the sequence X and returns sequence (a₁, ..., aₙ) of the same length and dimensionality; and  
...
"Transformers for Longer Sequences – Appendix"  

connected network \( O \) that takes in a vector in \( \mathbb{R}^d \) and returns a vector in \( \mathbb{R}^d \).  

A.1 Notation  

1. An attention mechanism ATTN that takes in the sequence X and returns sequence (a₁, ..., aₙ) of the same length and dimensionality; and
2. A two layer fully connected network O that takes in a vector in ℝᵈ and returns a vector in ℝᵈ.  
...
The text content from the image reads:
...
$$
```latex
\bar{f}(X) = \sum_{P \in \mathbf{G}_{\delta}^{E}} f(P - E) \cdot 1 \left[ \| \text{ReLU}(X - P) \|_{\infty} \leq \delta \right].
```
$$  
...
Therefore, it suffices to approximate a function \( \bar{f}: \mathbf{G}_{\delta}^{E} \to \mathbb{R}^{n \times d} \) defined as  

$$
```latex
E = \begin{bmatrix}
0 & 0 & 0 & \cdots & 0 \\
\delta^{-d} & \delta^{-d} & \delta^{-d} & \cdots & \delta^{-d} \\
\delta^{-2d} & \delta^{-2d} & \delta^{-2d} & \cdots & \delta^{-2d} \\
\vdots & \vdots & \vdots & \ddots & \vdots \\
\delta^{-(n-1)d} & \delta^{-(n-1)d} & \delta^{-(n-1)d} & \cdots & \delta^{-(n-1)d}
\end{bmatrix}
```
$$  
...
\psi_u(Z; b_1, b_2)_i = 
\begin{cases} 
(\max_{j \in N(i)} u^T Z_j - \min_{j \in N(i)} u^T Z_j)e_1 & \text{if } b_1 \leq u^T Z_j \leq b_2 \\
0 & \text{else}.
\end{cases}
```
$$  

Note that \( e_1 \in R^{d+1} \) denotes \( (1, 0, \ldots, 0) \).  

$$
```latex
\tilde{\psi}(X,b)_i = \sigma_H \left[ (u^T \cdot X_i)^T \cdot (u^T X_{N(i)} - b1_{N(i)}^T) e^{(1)} (u^T X_{N(i)}) \right]
```
$$  

1. For any \( P \in \mathbf{G}_{\delta}^{E} \), \( q(P) \) contains distinct entries.  

The function, which can be implemented by a sparse attention mechanism  
...
This will render the text exactly as shown in the image.
$$  
...
For \( i \in \{0, 1, \ldots, n\} \), we will use \( l_i \) to denote the inner products \( \langle u, x_i \rangle \) at the beginning. For \( f_i = \langle u, x_i \rangle \) after the \( i^{th} \) column has changed for \( i \in \{1, \ldots, n\} \) and we will use \( f_0^k \) to denote \( \langle u, x_0 \rangle \) after the \( k^{th} \) phase. We need to distinguish the global token further as its inner product will change in each phase. Initially, given \( X \in \mathbf{G}_\delta^\frac{E}{2} \), the following are true:  

To successfully encode the entire context in each token, we will interleave the shift operator to target the original columns 1, . . . , n and to target the global column 0. After a column i is targeted, its inner product with u will encode the entire context of the first i columns. Next, we will shift the global token to take this context into account. This can be subsequently used by the remaining columns.  
...
Proof. Define \( u \in \mathbb{R}^{d+1} = [1, \delta^{-1}, \delta^{-2}, \ldots, \delta^{-d+1}, \delta^{-nd}] \) and let \( X_0 = (0, \ldots, 0, 1) \). We will assume that \( \langle x_i, x_0 \rangle = 0 \), by assuming that all the columns \( x_1, \ldots, x_n \) are appended by 0.  
...
We do this in phases indexed from \( i \in \{1, \ldots, n\} \). Each phase consists of two distinct parts:
...
```latex
\begin{align*}
\delta^{-(i-1)d} &\leq \langle u, X_i \rangle \leq \delta^{-id} - \delta && \text{for all } i \in [n] \\
...
\tilde{\psi}(Z;b)_i = 
\begin{cases} 
(\max_{j \in N(i)} u^T Z_j)e_1 & \text{if } u^T Z_j > b \\
(\min_{j \in N(i)} u^T Z_j)e_1 & \text{if } u^T Z_j < b
\end{cases}
```
$$  

the Key, Query and Value functions are simply affine transformations of X  
...
$$
l _ { k } + 1 < l _ { k + 2 } \cdots < l _ { n } < f _ { 1 } < f _ { 2 } < \cdots < \tilde { f } _ { 0 } ^ { k - 1 } < f _ { k }
$$  

The order of the inner products after \( k^{th} \) phase is  

$$
l _ { k + 1 } < l _ { k + 2 } \cdots < l _ { n } < f _ { 1 } < f _ { 2 } < \cdots < f _ { k } < \tilde { f } _ { 0 } ^ { k } .
$$  
...
The base case for \( k = 0 \) is trivial as we simply set \( S_0 = \delta^{-(n+1)d} \), \( T_0 = \delta^{-(n+1)d} + \delta \).  

The base case is \( k = 0 \), which is trivial as we simply set \( S_0 = \delta^{-(n+1)d} \) and \( T_0 = \delta^{-(n+1)d} + \delta \). The first nontrivial case is \( k = 1 \).  

1. \( S_k < \tilde{f}_0^k < T_k \) for all \( k \in \{0, 1, \ldots, n\} \).
2. \( T_{k-1} \leq f_k < S_k \)  

The base case for \( k = 0 \) is trivial as we simply set \( S_0 = \delta^{-(n+1)d} \), \( T_0 = \delta^{-(n+1)d} + \delta \). The first nontrivial case is \( k = 1 \).  
...
This lemma describes how to construct a specific function \( g_v \) using feed-forward neural network layers, given certain conditions on the number of layers and the activation functions used.  

The previous lemma shows that we can compute a contextual mapping using only sparse transforms. We now use the following lemma to show that we can use a contextual mapping and feed-forward layers to accurately map to the desired output of the function \( f \).  
...
The previous section assumed we used Transformers that used hardmax operator σ_H and activations functions belonging to the set Φ. This is without loss of generality as following lemma shows.  

A.2.3 Approximating modified Transformers by Transformers  

Theorem 2. Let \(1 \leq p \leq \infty\) and \(\epsilon > 0\), there exists a transformer network \(g \in \mathcal{T}^{2,1,4}_D\) which achieves a ratio of \(d_p(f,g) \leq \epsilon\) where \(D\) is the sparse graph.  

$$
g _ { v } ^ { t k n } ( g _ { c } ( L ) _ { j } ) = f ( L ) _ { j }
$$  

Combining the above lemma with Lemma 3, we get our main result:  
...
Transformer Decoder We need both an encoder and a decoder in the transformer for simulating a Turing machine. We utilize the same notation used in App. A.1 for encoders. The decoder is similar to an encoder but with additional attention to an external pair of key-value vectors (Ke ∈ ℝn×m, Ve ∈ ℝn×d), which usually come from the encoder stack. A single layer of Transformer decoder is a parametric function Dec receiving a sequence Yj = (y1, . . . , yj) of vectors in ℝd plus the external (Ke, Ve) and returning a sequence of vectors Zj = (z1, . . . , zj) of the same length. Each zi is a d dimensional vector as well. Dec has three components, one more than Enc:  

The text content from the image is as follows:

"Vector representations For a symbol s ∈ Σ, [s] denotes its one-hot vector representation in Q^|Σ|. All the transformer intermediate vectors used in our simulations have dimension d = 2|Q| + 4|Σ| + 16. Note that we use five extra dimensions as compared to Pérez et al. [72]. We follow the convention used in Pérez et al. [72] and write a vector v ∈ Q^d arranged in four groups of values as follows."  
...
"ATTN_D and O are as defined in App. A.1 and it remains to define CROSSATTN. The i^{th} output vector of multi-head cross-attention attention is given by"  

The text content from the image is:

"Turning Machine We will use the same setup of Turing Machine that was used by Pérez et al. [72] (see section B.4). Given a Turing Machine \( M = (Q, \Sigma, \delta, q_{init}, F) \), we use the following notation"  

B.2 Details of the Simulation  

$$
```latex
\text{CROSSATTN}(\mathbf{Y}_j)_i = \sum_{h=1}^{H} \sigma \left( (\mathbf{y}_i W_Q^h)(\mathbf{K}^{(e)} W_K^h)^T \right) \cdot (\mathbf{V}^{(e)} W_V^h)
```
$$  
...
where \( q_i \in \mathbb{Q}^{|Q|} \), \( s_i \in \mathbb{Q}^{|\Sigma|} \), and \( x_i \in \mathbb{Q} \).  

B Turing Completeness  

$$
p _ { i } = \mathrm { A T T N } _ { D } ( Y _ { j } ) _ { i } + y _ { i }
$$  

$$
\boldsymbol { a } _ { i } = \operatorname { C R O S S A T N } ( p _ { i } , K ^ { e } , V ^ { e } ) + p _ { i }
$$  
...
2. A cross-attention mechanism CROSSATTN that takes in the sequence (p₁, ..., pⱼ) plus the external (Kₑ, Vₑ) and returns sequence (a₁, ..., aⱼ), with each aᵢ ∈ ℝᵈ; and  

3. A two layer fully connected network \( O \) that takes in a vector in \( \mathbb{R}^d \) and returns a vector in \( \mathbb{R}^d \). Then \( i \)-th output vector of \( \text{Dec}(Y_j; K^e, V^e) \) is computed as follows:  

$$
```latex
\boldsymbol{v} = \left[ \begin{array}{ccccccccc}
\boldsymbol{q}_1, \boldsymbol{s}_1, x_1, \\
\boldsymbol{q}_2, \boldsymbol{s}_2, x_2, x_3, x_4, x_5, x_6, \\
\boldsymbol{s}_3, x_7, \boldsymbol{s}_4, \\
x_8, x_9, x_{10}, x_{11}, x_{12}, x_{13}, x_{14}, x_{15}, x_{16}
\end{array} \right]
```
$$  

1. An attention mechanism ATTN that takes in the sequence \( \mathbf{Y}_j \) and returns sequence \( (\mathbf{p}_1, ..., \mathbf{p}_j) \) of the same length and dimensionality;
2. A cross-attention mechanism CROSSATTN that takes in the sequence \( (\mathbf{p}_1, ..., \mathbf{p}_j) \) plus the external \( (\mathbf{K}^{\text{e}}, \mathbf{V}^{\text{e}}) \) and returns sequence \( (\mathbf{a}_1, ..., \mathbf{a}_j) \), with each \( \mathbf{a}_i \in \mathbb{R}^d \); and
3. A two layer fully connected network \( O \) that takes in a vector in \( \mathbb{R}^d \) and returns a vector in \( \mathbb{R}^d \). Then \( i \)-th output vector of Dec\( (\mathbf{Y}_j; \mathbf{K}^{\text{e}}, \mathbf{V}^{\text{e}}) \) is computed as follows:  

3. A two layer fully connected network \( O \) that takes in a vector in \( \mathbb{R}^d \) and returns a vector in \( \mathbb{R}^d \).  
...
$$
{ \bigg ( } { \frac { j ( j + 1 ) } { 2 } } + k , { \frac { k ( k + 1 ) } { 2 } } { \bigg ) }
$$  

$$
( { \frac { j ( j + 1 ) } { 2 } } + k , { \frac { j ( j + 1 ) } { 2 } } + k ) { \mathrm { ~ i f ~ } } k > 1 { \mathrm { ~ e l s e ~ } } \left( { \frac { j ( j + 1 ) } { 2 } } + 1 , { \frac { j ( j + 1 ) } { 2 } } \right) .
$$  
...
\mathbf{y}_i = 
\begin{bmatrix}
\lfloor q^{g(i)} \rfloor, \lfloor s^{g(i)} \rfloor, c^{g(i)}, \\
0, \ldots, 0, \\
\mathbf{0}_s, 0, \lfloor w^{(i)} \rfloor, \\
0, 0, 0, 0, u_1^{(i)}, u_2^{(i)}, u_3^{(i)}, u_4^{(i)}
\end{bmatrix}
```
$$  

$$
y _ { 1 } \ = \ [ \ [ q _ { \mathrm { i n i t } } ] , [ \# ] , 0 ,
$$  

where \( g(i) = \left\lfloor \frac{-1 + \sqrt{1 + 8i}}{2} \right\rfloor \) and \( h(i) = g(i + 1) - g(i) \). Note that \( h(i) \) reduces to a binary indicator variable \( 1 \left\{ \frac{-1 + \sqrt{1 + 8i}}{2} = \left\lfloor \frac{-1 + \sqrt{1 + 8i}}{2} \right\rfloor \right\} \).  

The text content from the image is:

"B.2.1 Layer 1: Simulate Transition Function"  

We next show how to construct the decoder layers to produce the sequence of outputs \( y_1, y_2, \ldots \), where \( y_i \) is given by:  

The text content from the image is:

"Embeddings and positional encodings Our construction needs a different positional encoding posDec : N → Q^d for decoder:"  
...
"Since \( p_i^1 \) is of the form [_, ..., _, 1, g(i) + 1, _, ..., _], we know by Lemma B.1 of Pérez et al. [72] that if we use \( p_i^1 \) to attend over the encoder we obtain"  

$$
```latex
\overline{\boldsymbol{y}}_{i} = \left[ \begin{array}{c}
...
$$
```latex
\text{CROSSATTN}(p_i^1, K^e, V^e) = 
\begin{bmatrix}
0, \ldots, 0, \\
0, \ldots, 0, \\
[\alpha^{g(i)+1}], \beta^{g(i)+1}, \mathbf{0}_s, \\
0, \ldots, 0
\end{bmatrix}
```
$$  
...
$$
{ \boldsymbol { y } } _ { 1 } + \operatorname { p o s } _ { \mathrm { D e c } } ( 1 ) , \, { \boldsymbol { y } } _ { 2 } + \operatorname { p o s } _ { \mathrm { D e c } } ( 2 ) , \, \ldots , \, { \boldsymbol { y } } _ { r } + \operatorname { p o s } _ { \mathrm { D e c } } ( r ) .
$$  

$$
```latex
\text{pos}_{\text{Dec}}(i) \quad = \quad [ \begin{array}{c}
0, \ldots, 0, \\
0, \ldots, 0, \\
0, \ldots, 0, \\
...
\[
\overline{y}_i \text{ the vector } y_i \text{ plus its positional encoding. Thus we have } \forall 1 \leq i \leq r \text{ th}
\]  
...
"Lemma 6 (Lemma B.2 [72]). There exists a two-layer feed-forward network \( O_1 : \mathbb{Q}^d \to \mathbb{Q}^d \) such that with input vector \( \alpha_i^1 \) (Eq. (7)) produces as output"  

$$
```latex
z_{i}^{2} = O_{2}(a_{i}^{2}) + a_{i}^{2} = \begin{bmatrix}
\left\lfloor q^{g(i)} \right\rfloor, \left\lfloor s^{g(i)} \right\rfloor, c^{g(i)}, \\
\left\lfloor q^{g(i)+1} \right\rfloor, \left\lfloor v^{g(i)} \right\rfloor, c^{g(i)+1}, \frac{1}{g(i)+1}, \frac{1}{(g(i)+1)^{2}}, \frac{c^{g(i)+1}}{g(i)+1}, \frac{c^{g(i)}}{g(i)+1}, \\
\left\lfloor \alpha^{g(i)+1} \right\rfloor, \beta^{g(i)+1}, \left\lfloor w^{(i)} \right\rfloor, \\
1, g(i) + 1, \frac{1}{g(i)+1}, \frac{1}{(g(i)+1)^{2}}, h(i), u_{1}^{(i)}, u_{2}^{(i)}, u_{3}^{(i)}, u_{4}^{(i)}
\end{bmatrix}
```
$$  
...
```latex
\begin{equation}
a_i^1 = \text{CROSSATTN}(p_i^1, K^e, V^e) + p_i^1
= \left[
\begin{array}{c}
\llbracket q^{g(i)} \rrbracket, \llbracket s^{g(i)} \rrbracket, c^{g(i)}, \\
0, \ldots, 0, \\
...
"where α and β are as defined in Eq. (21) of [72]. Thus in Eq. (4) we finally produce the vector \(a_i^1\) given by"  

$$
O ( [ x , y , z , b ] ) ) = \left\{ \begin{array} { l l } { { [ 0 , 0 , 0 , 0 ] } } & { { \mathrm { i f ~ } b = 1 , } } \\ { { [ 0 , z - y , - z , 0 ] } } & { { \mathrm { i f ~ } b = 0 . } } \end{array} \right.
$$  

$$
```latex
z_{i}^{1} = O_{1}(a_{i}^{1}) + a_{i}^{1} \quad = \quad 
\begin{bmatrix}
\llbracket q^{g(i)} \rrbracket, \llbracket s^{g(i)} \rrbracket, c^{g(i)}, \\
...
$$
```latex
O_{1}(\boldsymbol{a}_{i}^{1}) = 
\left[
\begin{array}{cccccc}
0, \ldots, 0, \\
\lfloor q^{g(i)+1} \rfloor, \lfloor v^{g(i)} \rfloor, m^{g(i)}, 0, 0, 0, 0 \\
0, \ldots, 0 \\
0, \ldots, 0
\end{array}
\right]
...
Note the minimization is only over Turing machine steps, i.e., over compute nodes in our case. We show below that we can estimate \( m(j) \) by parts using sparse attention mechanism. The main idea is just to notice that minimization problem \( \min_{m \in \{0, \ldots, t\}} \chi_t^j \) can be expressed as \( \min \{\cdots \min \{\min \{\chi_0^j, \chi_1^j\}, \chi_2^j\}, \ldots, \chi_t^j\} \) by the associativity property.  

The text content from the image is:

"and similarly for u's. So after going through all the intermediate nodes, finally at the next compute node, i.e. when k = j + 1, we will obtain the minimum value over all of 0, 1, ..., j. This implies at a compute node will be able to recover ℓ(g(i) + 1) and its corresponding value as shown in Lemma B.4 of [72]. Then we have that p_i^A is given by"  
...
F(z_r^4) = 
\begin{bmatrix}
\llbracket q^{g(r+1)} \rrbracket, \llbracket s^{g(r+1)} \rrbracket, c^{g(r+1)}, \\
0, \ldots, 0, \\
\mathbf{0}_s, 0, \llbracket w^{(r+1)} \rrbracket, \\
0, 0, 0, 0, 0, u_1^{(r+1)}, u_2^{(r+1)}, u_3^{(r+1)}, u_4^{(r+1)}
\end{bmatrix} = y_{r+1}
```
$$  
...
$$
```latex
K_{4}(z_{i}^{3}) &= \left[\begin{array}{c}
0,\ldots,0\\
0,\ldots,0,\\
0,\ldots,0,\\
0,\hat{u}_{2}^{(i)},\hat{u}_{4}^{(i)},\hat{u}_{3}^{(i)},0,0,0,0,0
\end{array}\right]\\
V_{4}(z_{i}^{3}) &= \left[\begin{array}{c}
0,\ldots,0,\\
0,\ldots,0,\\
\mathbf{0}_{s},0,[\hat{v}^{(i)}],\\
0,0,0,0,0,\hat{u}_{1}^{(i)},\hat{u}_{2}^{(i)},\hat{u}_{3}^{(i)},\hat{u}_{4}^{(i)}
\end{array}\right]
```
$$  
...
$$
```latex
m(t) = \arg\min_{m \in \{0, \ldots, t\}} \chi_t^j = \arg\min_{m \in \{0, \ldots, t\}} |\langle Q_4(z_j^3), K_4(z_m^3) \rangle|
```
$$  
...
```latex
\begin{equation}
p_{i}^{4} = \text{ATTN}_{D}(Z_{i}^{3}) + z_{i}^{3}
= \left[
\begin{array}{c}
0, \ldots, 0, \\
...
1, \hat{u}_{1}^{(i)}, \hat{u}_{2}^{(i)}, \hat{u}_{3}^{(i)}, h(i), u_{1}^{(i+1)}, u_{2}^{(i+1)}, u_{3}^{(i+1)}, u_{4}^{(i+1)}
\end{array}
\right]
\end{equation}
```
...
The image contains a statement referred to as "Conjecture 1 (Orthogonal Vectors Conjecture)." It posits that for every positive ε, there exists a constant c ≥ 1 such that given n Boolean vectors in d dimensions, it is impossible to determine if there is a pair of orthogonal vectors in O(n^(2-ε)) time on instances where d is at least c log n.  

Finding vectors that are furthest apart boils down to minimizing inner product search in case of unit vectors. For a full-attention mechanism with appropriate query and keys, this task is very easy as we can evaluate all pair-wise inner products.  

To complete the argument, observe that it now only takes O(n) inner products to check if there is a pair of orthogonal vectors as we need only compare ⟨u_i, u_{i^*}⟩.  

The task described in the image is as follows:

**Task 1.** Given \( n \) unit vectors \(\{u_1, \ldots, u_n\}\), each in \(\mathbb{R}^d\) where \( d = \Theta(\log^2 n) \), compute \( f(u_1, \ldots, u_n) \to (u_{1*}, \ldots, u_{n*}) \) where for a fixed \( j \in [n] \), we define \( j^* = \arg \max_k \|u_k - u_j\|_2^2 \).  

Using conjecture 1, we show a reduction to show that a transformer \( g \in \mathcal{T}_{D}^{H=O(d), m=O(d), q=O(d)} \) for any sparse directed graph \( D \) which completes Task 1 must require a superlinear number of layers.  

$$
```latex
a_i = \text{Attn}(Q(x_i), K(X), V(X)) + x_i = [u_i; u_{\arg\max_j \langle -u_i, u_j \rangle}] = [u_i; u_{i^*}]
```
$$  

The text content from the image is as follows:

"Proposition 2. There exists a single layer full-attention network \( g \in \mathcal{T}^{H=1, m=2d, q=0} \) that can evaluate Task I, i.e., \( g(u_1, ..., u_n) = [u_1^*, ..., u_n^*] \), but for any sparse-attention network in \( \mathcal{T}_D^{H=O(d), m=O(d), q=O(d)} \) with graph \( D \) having \( \tilde{O}(n) \) edges (i.e., inner product evaluations), would require \( \tilde{\Omega}(n^{1-o(1)}) \) layers."  
...
"Then Attn(Q(x_i), K(X), V(X) = [0; u_{arg max_j} \langle -u_i, u_j \rangle]. Then,"  

$$
x _ { i } : = E ( u _ { i } ) = [ u _ { i } ; 0 ]
$$  

Step 3 Let \( O(a_i) = 0 \), then the output \( z_i = [u_i; u_i^*] \) as desired.  

The text content from the image is "C Limitations".  
...
$$
```latex
a_i = \text{Attn}(Q(x_i), K(X), V(X)) + x_i = [u_i; u_{\arg\max_j \langle -u_i, u_j \rangle}] = [u_i; u_i]
```
$$  
...
Part 2: Every Sparse Attention Mechanism will need \(\tilde{\Omega}(n^{1-\epsilon})\) layers that it is impossible to solve Task 1 by any \(g \in \mathcal{T}_D^{H=O(d), m=O(d), q=O(d)}\) sparse-attention graph \(D\) with \(\tilde{O}(n)\) edges.  
...
Thus, total computation per layer is Ō(n d^3) and consequently Ō(n l d^3) for the whole network consisting of l layers.  
...
"Recall, full dense attention scores can be calculated by simple matrix product of query and key matrix with a cost of O(n^2d), as illustrated in Fig. 4a. Now note that if we blockify the query and key matrix and multiply, then with only O(nbd) cost we will obtain the block diagonal portion of the attention score, as depicted in Fig. 4b. To elaborate this lets assume that Q, K ∈ ℝ^n×d are the query and key matrix corresponding to n tokens such that Q_i. = x_iW_Q and K_i. = x_iW_K. We reshape n × d query"  
...
3. Global attention: Global attention remains the same as defined in Sec. 2, but we compute it in terms of blocks. In Fig. 3c, \( g = 1 \) with block size 2. For BIGBIRD-ITC this implies that one query and key block attend to everyone.  
...
Thus, the result of all the three components is basically a compact dense tensor \( K'' \) of size \([n/b] \times (g + w + r)b \times d\) as shown in Fig. 6. Computing the final attention score then just boils down to a dense tensor multiplication, at which TPU/GPU are very efficient. Specifically, we need to multiply \( Q' \) (size: \([n/b] \times b \times d\) ) and \( K'' \) (size: \([n/b] \times (g + w + r)b \times d\) ) with a cost of \( O(n(g + w + r)bd) \) to yield the desired attention score tensor of size \([n/b] \times b \times (g + w + r)b\), which can be reshaped to obtain all the attention scores according to the BigBird pattern.  

The text content from the image is:

"matrix, Q, and key matrix, K, along the sequence length to obtain [n/b] × b × d tensors Q' and K' respectively. Now we multiply the two tensors as"  

$$
A _ { j s t } = \sum _ { u } Q _ { j s u } ^ { \prime } K _ { j t u } ^ { \prime } , \qquad j = 0 , 1 , . . . , [ n / b ]
$$  

The text content from the image is as follows:

"Figure 5: Construction of rolled key-block tensor. Make w copies of the key matrix. Index the copies as -(w-1)/2 ≤ j ≤ (w-1)/2. Roll jth copy by j blocks. Positive roll means circular shift entries left and likewise for negative roll corresponds to right shift. Finally, reshape by grouping the blocks along a new axis to obtain the key-blocked tensor. For illustration purpose w = 3 is chosen."  
...
The text content from the image is "E.1 MLM Pretraining".  
...
It is worthwhile to note that explicitly designed attention connection in ETC works slightly better, the random connection based ITC is pretty competitive.  
...
"decoder hyperparameters for Summarization. We use full attention in decode"  
...
The text content from the image is "F.1 Pretraining".  
...
The provided text content is: "We provide details of the experimental setup for BIGBIRD on genomics data."  

at random a starting point q between base pairs 0 and 5000 from the 5' end  

Sure, here is the text content from the image:

5. For each chromosome \( C \), repeat the following procedure 10 times.
...
Figure 9: Visual description of the DNA segment from which we predict the chromatin profile for a given non-coding region of the raw DNA sequences of GRCh37. We take 8000 bps of DNA before and after the given non-coding region as context. The complete fragment of DNA including the context on both sides, is then tokenized to form our input sequence of tokens. The task is to predict 919 chromatin profile including 690 transcription factors (TF) binding profiles for 160 different TFs, 125 DNase I sensitivity (DHS) profiles and 104 histone-mark (HM) profiles  
...
Each training sample consists of an 8,000-bp sequence from the human GRCh37 reference genome centered on each 200-bp bin and is paired with a label vector for 919 chromatin features. As before, we prefix and append each example with [CLS] and [SEP] token respectively. The output corresponding to the [CLS] token from BIGBIRD transformer encoder is fed to a linear layer with 919 heads. Thus we jointly predict the 919 independent binary classification problems. We fine-tune the pretrained BIGBIRD from App. F.1 using hyper-parameters described in Tab. 21. As the data is highly imbalanced data (way more negative examples than positive examples), we upweighted loss function for positive examples by factor of 8.  
...
"and Troyanskaya [109]8, to train BIGBIRD to predict the chromatic profile."  