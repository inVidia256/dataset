...
The text content from the image is "2.2 CONFIGURATIONS".  

The text content from the image is "2.3 DISCUSSION".  

The text content from the image is "2.1 ARCHITECTURE".  
...
The text content from the image is "3.1 TRAINING".  
...
The main evaluation criterion used in ILSVRC, and is computed as the proportion of images such that the ground-truth category is outside the top-5 predicted categories.  
...
The text content from the image is "4.1 SINGLE SCALE EVALUATION".  
...
The text content from the image is "4.4 CONVNET FUSION".  
...
"Loai, M. Regularized max pooling for image categorization. In Proc. BMVC., 2014"  
...
"with the state of the art in ILSVRC localisation. Our method is denote"  
...
The paper is converted to ICLR-2015 submission format. Also adds experiments with multiple crops for classification.  
