...
The text content from the image is "Jure Leskovec".  

The text content from the image is "William L. Hamilton*".  
...
The GraphSAGE algorithm is conceptually inspired by a classic algorithm for testing graph isomorphism. If, in Algorithm 1, we (i) set \( K = |\mathcal{V}| \), (ii) set the weight matrices as the identity, and (iii) use an appropriate hash function as an aggregator (with no non-linearity), then Algorithm 1 is an instance of the Weisfeiler-Lehman (WL) isomorphism test, also known as "naive vertex refinement" [32]. If the set of representations \(\{\mathbf{z}_v, \forall v \in \mathcal{V}\}\) output by Algorithm 1 for two subgraphs are identical then the WL test declares the two subgraphs to be isomorphic. This test is known to fail in some cases, but is valid for a broad class of graphs [32]. GraphSAGE is a continuous approximation to the WL test, where we replace the hash function with trainable neural network aggregators. Of course, we use GraphSAGE to generate useful node representations—not to test graph isomorphism. Nevertheless, the connection between GraphSAGE and the classic WL test provides theoretical context for our algorithm design to learn the topological structure of node neighborhoods.  

Algorithm 1 describes the embedding generation process in the case where the entire graph, \( G = (V, E) \), and features for all nodes \( x_v, \forall v \in V \), are provided as input. We describe how to generalize this to the minibatch setting below. Each step in the outer loop of Algorithm 1 proceeds as follows, where \( k \) denotes the current step in the outer loop (or the depth of the search) and \( h^k_v \) denotes a node’s representation at this step: First, each node \( v \in V \) aggregates the representations of the nodes in its immediate neighborhood, \( \{h^{k-1}_u, \forall u \in N(v)\} \), into a single vector \( h_{N(v)}^{k-1} \). Note that this aggregation step depends on the representations generated at the previous iteration of the outer loop (i.e., \( k-1 \)), and the \( k=0 \) (“base case”) representations are defined as the input node features. After aggregating the neighboring feature vectors, GraphSAGE then concatenates the node’s current representation, \( h_v^{k-1} \), with the aggregated neighborhood vector, \( h_{N(v)}^{k-1} \), and this concatenated vector is fed through a fully connected layer with nonlinear activation function \( \sigma \), which transforms the representations to be used at the next step of the algorithm (i.e., \( h_v^k, \forall v \in V \)). For notational convenience, we denote the final representations output at depth \( K \) as \( z_v \equiv h_v^K, \forall v \in V \). The aggregation of the neighbor representations can be done by a variety of aggregator architectures (denoted by the AGGREGATE placeholder in Algorithm 1), and we discuss different architecture choices in Section 3.3 below.  
...
The image contains an algorithm for a graph neural network (GNN) with the following specifications:
...
**Output:**
- Vector representations \( \mathbf{z}_v \) for all \( v \in \mathcal{V} \)
...
2. for k = 1...K do
3.     for v ∈ V do
4.         h^k_{\mathcal{N}(v)} ← AGGREGATE_k({h^{k-1}_u, ∀u ∈ \mathcal{N}(v)});
5.         h^k_v ← σ(W^k · CONCAT(h^{k-1}_v, h^k_{\mathcal{N}(v)}))
6.     end
...
This algorithm iteratively updates the node representations in a graph neural network over multiple layers, where each layer aggregates information from the neighborhood of a node and applies a non-linear transformation to update the node's representation.  

\`\`\`
Input : Graph G(V, E); input features {x_v, ∀v ∈ V}; depth K; weight matrices W^k, ∀k ∈ {1, ..., K}; non-linearity σ; differentiable aggregator functions AGGREGATE_k, ∀k ∈ {1, ..., K}; neighborhood function N : v → 2^V
Output: Vector representations z_v for all v ∈ V

1 h_v^0 ← x_v, ∀v ∈ V;
2 for k = 1...K do
3     for v ∈ V do
4         h_N(v)^k ← AGGREGATE_k({h_u^(k-1), ∀u ∈ N(v)});
5         h_v^k ← σ(W^k · CONCAT(h_v^(k-1), h_N(v)^k))
6     end
...
1: GraphSAGE embedding generation (i.e., forward propagation) algorithm  
...
fixed.³ That is, using overloaded notation, we define N(v) as a fixed-size, uniform draw from the set {u ∈ V : (u, v) ∈ E}, and we draw different uniform samples at each iteration, k, in Algorithm 1. Without this sampling the memory and expected runtime of a single batch is unpredictable and in the worst case O(|V|). In contrast, the per-batch space and time complexity for GraphSAGE is fixed at O(∏i=1K Si), where Si, i ∈ {1, ..., K} and K are user-specified constants. Practically speaking we found that our approach could achieve high performance with K = 2 and S1 · S2 ≤ 500 (see Section 4.4 for details).  
...
We call this modified mean-based aggregator convolutional since it is a rough, linear approximation of a localized spectral convolution [17]. An important distinction between this convolutional aggregator and our other proposed aggregators is that it does not perform the concatenation operation in line 5 of Algorithm 1—i.e., the convolutional aggregator does concatenate the node’s previous layer representation h_v^{k-1} with the aggregated neighborhood vector h_{\mathcal{N}(v)}^k. This concatenation can be viewed as a simple form of a “skip connection” [13] between the different “search depths”, or “layers” of the GraphSAGE algorithm, and it leads to significant gains in performance (Section 4).  

The text content from the image is as follows:

"where \( v \) is a node that co-occurs near \( u \) on fixed-length random walk, \( \sigma \) is the sigmoid function, \( P_n \) is a negative sampling distribution, and \( Q \) defines the number of negative samples. Importantly, unlike previous embedding approaches, the representations \( z_u \) that we feed into this loss function are generated from the features contained within a node's local neighborhood, rather than training a unique embedding for each node (via an embedding look-up)."  
...
The reference [17] likely refers to a specific paper or study on graph convolutional networks (GCNs) that uses a similar convolutional propagation rule.  
...
$$
J _ { \mathcal { G } } ( \mathbf { z } _ { u } ) = - \log \left( \sigma ( \mathbf { z } _ { u } ^ { \top } \mathbf { z } _ { v } ) \right) - Q \cdot \mathbb { E } _ { v _ { n } \sim P _ { n } ( v ) } \log \left( \sigma ( - \mathbf { z } _ { u } ^ { \top } \mathbf { z } _ { v _ { n } } ) \right) ,
$$  
...
The final aggregator we examine is both symmetric and trainable. In this pooling approach, each neighbor's vector is independently fed through a fully-connected neural network; following this transformation, an elementwise max-pooling operation is applied to aggregate information across the neighbor set:  

$$
```latex
\text{AGGREGATE}^{pool}_k = \max(\{\sigma\left(\mathbf{W}_{\text{pool}}\mathbf{h}^k_{u_i} + \mathbf{b}\right), \forall u_i \in \mathcal{N}(v)\}),
```
$$  
...
"Citation data. Our first task is predicting paper subject categories on a large citation dataset. We use an undirected citation graph dataset derived from the Thomson Reuters Web of Science Core Collection, corresponding to all papers in six biology-related fields for the years 2000-2005. The node labels for this dataset correspond to the six different field labels. In total, this dataset contains 302,424 nodes with an average degree of 9.15. We train all the algorithms on the 2000-2004 data and use the 2005 data for testing (with 30% used for validation). For features, we used node degrees and processed the paper abstracts according to Arora et al.'s [2] sentence embedding approach, with 300-dimensional word vectors trained using the GenSim word2vec implementation [30]."  
...
Figure 2: A: Timing experiments on Reddit data, with training batches of size 512 and inference on the full test set (79,534 nodes). B: Model performance with respect to the size of the sampled neighborhood, where the “neighborhood sample size” refers to the number of neighbors sampled at each depth for K = 2 with S1 = S2 (on the citation data using GraphSAGE-mean).  

4.1 Inductive learning on evolving graphs: Citation and Reddit data  

4.1 Inductive learning on evolving graphs: Citation and Reddit dat  
...
LSTM to mean; T = 4.5, p = 0.10, comparing pool to mean). There is no significant difference between the LSTM and pool approaches (T = 10.0, p = 0.46). However, GraphSAGE-LSTM is significantly slower than GraphSAGE-pool (by a factor of ≈2×), perhaps giving the pooling-based aggregator a slight edge overall.  
...
$$
| z _ { v } - c _ { v } | < \epsilon , \forall v \in \mathcal { V } ,
$$  
...
"Output values generated by Algorithm 1 and \( c_v \) are node clustering coefficient."  
...
Input : Graph G(V, E);
input features {x_v, ∀v ∈ B};
depth K; weight matrices W^k, ∀k ∈ {1, ..., K};
non-linearity σ;
differentiable aggregator functions AGGREGATE_k, ∀k ∈ {1, ..., K}
neighborhood sampling functions, N_k : v → 2^V, ∀k ∈ {1, ..., K}

Output: Vector representations z_v for all v ∈ B
...
9 for k = 1...K do
10 | for u ∈ B^k do
11 || h_N(u) ← AGGREGATE_k({h^{k-1}_{u'}, ∀u' ∈ N_k(u)});
12 || h^k_u ← σ(W^k · CONCAT(h^{k-1}_u, h_N(u)));
13 || h^k_u ← h^k_u / ||h^k_u||_2;
14 | end
15 end
16 z_u ← h^K_u, ∀u ∈ B  

The image contains a list of inputs for a graph neural network (GNN) model. Here is the text content extracted from the image:
...
"where Q ∈ ℝ^(d×d) is any orthogonal matrix. Since the embeddings are otherwise unconstrained and the only error signal comes from the orthogonally-invariant objective (4), the entire embedding space is free to arbitrarily rotate during training."  
...
The original graph is extremely dense. All experiments are conducted on the downsampled version, but we release the full version on the project website for reference.  

$$
\alpha \sum _ { i , j \in A } f ( \mathbf { z } _ { i } ^ { \top } \mathbf { z } _ { j } ) + \beta \sum _ { i , j \in B } g ( \mathbf { z } _ { i } ^ { \top } \mathbf { z } _ { j } )
$$  

$$
\mathbf { Z Q } ^ { \top } \mathbf { Q Z } ^ { \top } = \mathbf { Z Z } ^ { \top } ,
$$  
...
Proof. Many such functions exist. For concreteness, we provide one construction that satisfies these criteria. Let x ∈ U denote an arbitrary input to g, let dv = ||x − xv||2, ∀v ∈ D, and let g be defined as g(x) = ∑v∈D gv(x) with  

The text content from the image is:

"Lemma 1. Let \( C \in \mathbb{R}^+ \) be a fixed positive constant. Then for any non-empty finite subset of nodes \( \mathcal{D} \subseteq V \), there exists a continuous function \( g : U \to \mathbb{R} \) such that"  

$$
g _ { v } ( \mathbf { x } ) = \frac { 3 | \mathcal { D } | \epsilon } { b d _ { v } ^ { 2 } + 1 } - 2 \epsilon
$$  

To prove Theorem 1, we first prove three lemmas:  

Following Theorem 1, we let x_v ∈ U, ∀v ∈ V denote the feature inputs for Algorithm 1 on graph G = (V, E), where U is any compact subset of ℝ^d.  

where ε < 0.5 is a chosen error tolerance.  
...
```latex
\begin{cases}
g(\mathbf{x}) > \epsilon, & \text{if } \| \mathbf{x} - \mathbf{x}_v \|_2 = 0 \text{ for some } v \in \mathcal{D} \\
g(\mathbf{x}) \leq -\epsilon, & \text{if } \| \mathbf{x} - \mathbf{x}_v \|_2 > C, \forall v \in \mathcal{D},
\end{cases}
```
$$  
...
We also rely on fact that the max-pooling operator (with at least one hidden layer) is capable of approximating any Hausdorff continuous, symmetric function to an arbitrary ε precision [29].  
...
"Note also that \( g \) is continuous on its domain (\( d_v \in \mathbb{R}^+ \)) since it is the sum of finite set of continuous functions. Moreover, we have that, for a given input \( x \in U \), if \( d_v \geq C \) for all points \( v \in \mathcal{D} \) then \( g(x) = \sum_{v \in \mathcal{D}} g_v(\mathbf{a}) \leq -\epsilon \) by property 3 above. And, if \( d_v = 0 \) for any \( v \in \mathcal{D} \), then \( g \) is positive by construction, by properties 1 and 2, since in this case,"  

The function \( g : U \to \mathbb{R} \) can be approximated to an arbitrary degree of precision by standard multilayer perceptron (MLP) with at least one hidden layer and a non-constant monotonically increasing activation function (e.g., a rectified linear unit). In precise terms, if we let \( f_{\theta_\sigma} \) denote this MLP and \( \theta_\sigma \) its parameters, we have that for all \( \epsilon > 0 \), there exists \( \theta_\sigma \) such that \( |f_{\theta_\sigma}(\mathbf{x}) - g(\mathbf{x})| < \epsilon \) for all \( \mathbf{x} \in U \).  
...
$$
```latex
g_v(\mathbf{x}) + \sum_{v' \in \mathcal{D} \setminus v} g_{v'}(\mathbf{x}) \geq g_v(\mathbf{x}) - (|\mathcal{D}| - 1)2\epsilon
...
$$
\left\{ \begin{array} { l l } { f _ { \theta _ { \sigma } } ( \mathbf { x } _ { v } ) > 0 , } & { \forall v \in \mathcal { D } } \\ { f _ { \theta _ { \sigma } } ( \mathbf { x } _ { v } ) < 0 , } & { \forall v \in \mathcal { V } \setminus \mathcal { D } . } \end{array} \right.
$$  
...
$$
```latex
\mathbf{h}_{v}^{1} \neq \mathbf{h}_{v^{\prime}}^{1}, \forall (v, v^{\prime}) \in \left\{(v, v^{\prime}): \exists u \in \mathcal{V}, v, v^{\prime} \in \mathcal{N}^{2}(u)\right\}, \mathbf{h}_{v}^{1}, \mathbf{h}_{v^{\prime}}^{1} \in \mathcal{E}_{I}^{\chi(\mathcal{G}^{4})}
```
$$  
...
3. \frac{3|\mathcal{D}| \epsilon}{bd_v^2 + 1} - 2\epsilon \leq -\epsilon \text{ if } d_v \geq C.
```
$$  

$$
```latex
\mathbf{h}_{v}^{1} \neq \mathbf{h}_{v^{\prime}}^{1}, \forall (v, v^{\prime}) \in \{(v, v^{\prime}) : \exists u \in \mathcal{V}, v, v^{\prime} \in \mathcal{N}^{2}(u)\}, \mathbf{h}_{v}^{1}, \mathbf{h}_{v^{\prime}}^{1} \in \mathcal{E}_{I}^{\chi(\mathcal{G}^{4})}
```
$$  

2. lim\(_{d_v \to \infty} \left( \frac{3|\mathcal{D}| \epsilon}{bd_v^2 + 1} - 2\epsilon \right) = -2\epsilon

3. \(\frac{3|\mathcal{D}| \epsilon}{bd_v^2 + 1} - 2\epsilon \leq -\epsilon\) if \(d_v \geq C\).  
...
This will render the mathematical expression correctly in a LaTeX document.
$$  
...
"and since this is a continuous function of h_v^3, we can approximate it to an arbitrary ε precision with a single-layer MLP (or equivalently, one more iteration of Algorithm 1, ignoring neighborhood information). Again this last step follows directly from [15]."

This statement suggests that the function being discussed can be approximated using a single-layer Multi-Layer Perceptron (MLP) to any desired level of precision ε. This approximation is equivalent to performing one additional iteration of a specific algorithm (Algorithm 1), but without considering neighborhood information. The reference [15] indicates that this final step is derived from a previous work or study.  

$$
```latex
\mathbf{h}_v^3 = \mathbf{h}_v^1 \oplus \mathbf{A}_v \oplus \left( \sum_{v \in \mathcal{N}(v)} \mathbf{h}_v^1 \oplus \mathbf{A}_v \right).
```
$$  
...
"Letting \( m \) denote the dimensionality of the \( \mathbf{h}_{\nu}^{1} \) vectors (i.e., \( m \equiv \chi(G^{4}) \) from Lemma 3) and using square brackets to denote vector indexing, we can observe that"  

$$
| z _ { v } - c _ { v } | < \epsilon , \forall v \in { \mathcal V } ,
$$  

Thus, we have that \(\mathbf{b}^\top\mathbf{c}\) is the number of edges in the subgraph containing only \(v\) and its immediate neighbors and \(\sum_{i=0}^{m}\mathbf{b}[i] = d_v\). Finally we can compute  

$$
```latex
\frac{2(\mathbf{b}^\top \mathbf{c} - d_v)}{(d_v)(d_v-1)} = \frac{2|\{e_{v,v'} : v, v' \in \mathcal{N}(v), e_{v,v'} \in \mathcal{E}\}|}{(d_v)(d_v-1)} = c_v,
```
$$  

The text content from the image is:

- c ≡ h_v^3[3m : 4m] is the sum of the adjacency rows of v's neighbors.  

The text content from the image is:

- b ≡ h_v^3[m : 2m] is v's row in the adjacency matrix, A.  

- a ≡ h_v^3[0 : m] is v's one-hot indicator vector.
- b ≡ h_v^3[m : 2m] is v's row in the adjacency matrix, A.
- c ≡ h_v^3[3m : 4m] is the sum of the adjacency rows of v's neighbors  
...
Corollary 2. Suppose we sample nodes features from any probability distribution μ over x ∈ U, where μ is absolutely continuous with respect to the Lebesgue measure. Then the conditions of Theorem 1 are almost surely satisfied with feature inputs x_v ∼ μ.  
...
$$
```latex
\mathbf{h}_{v}^{k} \neq \mathbf{h}_{v^{\prime}}^{k}, \forall (v, v^{\prime}) \in \{(v, v^{\prime}): \exists u \in \mathcal{V}, v, v^{\prime} \in \mathcal{N}^{3}(u)\}, \mathbf{h}_{v}^{k}, \mathbf{h}_{v^{\prime}}^{k} \in \mathcal{E}_{I}^{d},
```
$$  