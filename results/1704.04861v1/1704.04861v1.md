...
"D_F × M feature map F and produces a D_F × D_F × N feature map G where D_F is the spatial width and height of a square input feature map^1, M is the number of input channels (input depth), D_G is the spatial width and height of a square output feature map and N is the number of output channel (output depth)."  
...
This will render the equation correctly in a LaTeX document.
$$  
...
$$
{ \hat { \mathbf { G } } } _ { k , l , m } = \sum _ { i , j } { \hat { \mathbf { K } } } _ { i , j , m } \cdot \mathbf { F } _ { k + i - 1 , l + j - 1 , m }
$$  
...
$$
D _ { K } \cdot D _ { K } \cdot M \cdot D _ { F } \cdot D _ { F } + M \cdot N \cdot D _ { F } \cdot D _ { F }
$$  
...
The second hyper-parameter to reduce the computational cost of a neural network is a resolution multiplier ρ. We apply  
...
The text content from the image is "4.1. Model Choices".  
...
The text content from the image is "4.7. Face Embeddings".  

The text content from the image is "4.6. Object Detection".  

The text content from the image is "4.5. Face Attributes".  
...
J. Krause, B. Sapp, A. Howard, H. Zhou, A. Toshev, T. Duerig, J. Philbin, and L. Fei-Fei.
