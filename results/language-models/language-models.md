...
The text in the image reads: "Language Models are Unsupervised Multitask Learners".  
...
$$
p ( x ) = \prod _ { i = 1 } ^ { n } p ( s _ { n } | s _ { 1 } , . . . , s _ { n - 1 } )
$$  
...
The text content from the image is "2.1. Training Dataset".  
...
The text content from the image is "2.2. Input Representation".  

The text content from the image is "3.1. Language Modeling".  
...
The text content from the image is "3.3. LAMBADA".  

The text content from the image is:

"Language Models are Unsupervised Multitask Learners"  

The text content from the image is "3.2. Children's Book Test".  
...
The text content from the image is "3.6. Summarization".  

3.7. Translation  

The text content from the image is "3.4. Winograd Schema Challenge".  
...
The text content from the image is "3.8. Question Answering".  
...
On reading comprehension, the performance of GPT-2 is competitive with supervised baselines in a zero-shot setting. However, on other tasks such as summarization, while it is qualitatively performing the task, its performance is still only rudimentary according to quantitative metrics. While suggestive as a research result, in terms of practical applications, the zero-shot performance of GPT-2 is still far from usable.  
...
The training data and capacity of GPT-2 is sufficient to overcome the inefficiencies of uni-directional representations demonstrated by BERT (Devlin et al., 2018).  
...
Bowman, S. R., Pavlick, E., Grave, E., Van Durme, B., Wang, A., Hula, J., Xia, P., Pappagari, R., McCoy, R. T., Patel, R., et al.
...
Paperno, D., Kruszewski, G., Lazaridou, A., Pham, Q. N., Bernardi, R., Pezzelle, S., Baroni, M., Boleda, G., and Fernández, R. The lambda dataset: Word prediction requiring a broad discourse context. arXiv preprint arXiv:1606.06031, 2016.
...
arXiv preprint arXiv:1808.07042, 2018.

Schwartz, R., Sap, M., Konstas, I., Zilles, L., Choi, Y., and Smith, N. A.
Story cloze task: Uw nlp system.
In Proceedings of the 2nd Workshop on Linking Models of Lexical, Sentential and Discourse-level Semantics, pp. 52–55, 2017.
...
Trichelair, P., Emami, A., Cheung, J. C. K., Trischler, A., Suleman, K., and Diaz, F.
On the evaluation of common-sense reasoning in natural language understanding.
arXiv preprint arXiv:1811.01778, 2018.
...
The text content from the image is "8.1. Model capacity".  
...
The text content from the image is "Outfit 2: Casual Jacket".  

I know that some people might be opposed to wearing sneakers in a turtleneck, but I wanted to be true to myself, so I went with a slim fitting turtleneck in a color more similar to my favorite color of the day. Overall, it’s a classic turtleneck, with nothing too flashy. I did swap my shoes for these ASICS Tiger Killshots and I wish I hadn’t done that. The shoe itself is very comfortable, however, I found that wearing them in these sneakers made my feet look a bit larger. I’d wear them with denim, chinos, whatever, but would never recommend wearing them alone. There are just too many variables involved.  
...
The text content from the image is "Small model completion".  
...
The text content from the image is "GPT-2 completion".  
...
In Cygwin on Windows, I found that I could not set the permissions of my \`.ssh/\` folder to be \`0600\`, as is required for ssh to allow you to use keys. The symptom I had was that no matter what I did, it always modified the owner and the group.  
...
The theme was "What was the theme".  
...
Model answer: Everest
Turker answers: unknown. ves. Yes. yes  
