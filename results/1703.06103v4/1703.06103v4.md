...
"Functional Data with Graph Convolutional Networks"  
...
The text content in the image is "Peter Bloem".  

The text content from the image is "Rianne van den Berg".  
...
The text content from the image is "Ivan Titov".  
...
where \(\mathcal{N}_{i}^{r}\) denotes the set of neighbor indices of node \(i\) under relation \(r \in \mathcal{R}\). \(c_{i,r}\) is a problem-specific normalization constant that can either be learned or chosen in advance (such as \(c_{i,r} = |\mathcal{N}_{i}^{r}|\)).  

We introduce the following notation: we denote directed and labeled multi-graphs as \( G = (\mathcal{V}, \mathcal{E}, \mathcal{R}) \) with nodes (entities) \( v_i \in \mathcal{V} \) and labeled edges (relations) \( (v_i, r, v_j) \in \mathcal{E} \), where \( r \in \mathcal{R} \) is a relation type.  
...
$$
W _ { r } ^ { ( l ) } = \sum _ { b = 1 } ^ { B } a _ { r b } ^ { ( l ) } V _ { b } ^ { ( l ) } ,
$$  

$$
h _ { i } ^ { ( l + 1 ) } = \sigma \left( \sum _ { m \in \mathcal { M } _ { i } } g _ { m } ( h _ { i } ^ { ( l ) } , h _ { j } ^ { ( l ) } ) \right) ,
$$  
...
The text content from the image is "2.2 Regularization".  
...
$$
W _ { r } ^ { ( l ) } = \bigoplus _ { b = 1 } ^ { B } Q _ { b r } ^ { ( l ) } .
$$  
...
Thereby, \( W_r^{(l)} \) are block-diagonal matrices: diag\( (Q_{1r}^{(l)}, \ldots, Q_{Br}^{(l)}) \) with \( Q_{br}^{(l)} \in \mathbb{R}^{(d^{(l+1)}/B) \times (d^{(l)}/B)} \).  

3 Entity classification  

The text content from the image is "4 Link prediction".  
...
$$
{ \mathcal { L } } = - { \frac { 1 } { ( 1 + \omega ) | { \hat { \mathcal { E } } } | } } \sum _ { ( s , r , o , y ) \in { \mathcal { T } } } y \log l \bigl ( f ( s , r , o ) \bigr ) + ( 1 - y ) \log \bigl ( 1 - l \bigl ( f ( s , r , o ) \bigr ) \bigr ) \, ,
$$  
...
We use the Adam optimizer (Kingma and Ba 2014) with a learning rate of 0.01. For the baseline and the other factorizations, we found the parameters from Trouillon et al. (2016) – apart from the dimensionality on FB15k-237 – to work best, though to make the systems comparable we maintain the same number of negative samples (i.e., ω = 1). We use full-batch optimization for both the baselines and our model.  
...
Table 4: Results on the Freebase and WordNet datasets. Results marked (*) taken from Trouillon et al. (2016). Results marks (***) taken from Nickel, Rosasco, and Poggio (2015). R-GCN+ denotes an ensemble between R-GCN and DistMult – see main text for details.  
...
The text content from the image is "7 Conclusions".  
...
Xiong, C., and Callan, J. 2015b. Query expansion with freebase. In Proceedings of the 2015 International Conference on The Theory of Information Retrieval, 111–120.
...
Paulheim, H., and Fümkranz, J. 2012. Unsupervised generation of data mining features from linked open data. In Proceedings of the 2nd international conference on web intelligence, mining and semantics, 31.  
...
For WL, we use the tree variant of the Weisfeiler-Lehman subtree kernel from the Mustard library. For RDF2Vec, we use an implementation provided by the authors of (Ristoski and Paulheim 2016) which builds on Mustard. In both cases, we extract explicit feature vectors for the instance nodes, which are classified by a linear SVM.  
...
The text in the image reads: "Parameter choices based on validation set performance for 2-layer R-GCN model."  

Parameter choices based on validation set performance for 2-layer R-GCN model  
