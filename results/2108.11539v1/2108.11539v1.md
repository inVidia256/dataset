...
"Object detection technology on drone-captured scenarios has been widely used in many practical applications, such as plant protection [18, 41], wildlife protection [23, 22] and urban surveillance [1, 15]. In this paper, we focus on improving."  
...
improving the performance of object detection on drone-captured images and providing insight for the above-mentioned numerous applications.  

The text content from the image is:

"Recent years have witnessed significant progresses in object detection tasks using deep convolutional neural networks [40, 37, 34, 27, 58]. Some notable benchmark datasets like MS COCO [30] and PASCAL VOC [9] greatly"  
...
To further improve the performance of TPH-YOLOv5, we employ bag of tricks (Fig.2). Specifically, we adopt data augmentation during training, which promotes adaptation for dramatic size changes of objects in images. We  
...
"Figure 3. The architecture of the TPH-YOLOv5.
a) CSPDarknet53 backbone with three transformer encoder blocks at the end.
b) The Neck use the structure like PANet.
c) Four TPHs (transformer prediction heads) use the feature maps from transformer encoder blocks in Neck. In addition, the number of each block is marked with orange numbers on the left side of the block."  
...
The text content from the image is "3.2. TPH-YOLOv5".  
...
We scale the test images to three different sizes in m-testing, and then flip them horizontally, so that a total of 6 different images are obtained. After testing six different images and fusing the results, we get the final test result.  
...
The text content from the image is "6. Acknowledgments".  
...
The text in the image reads: "PH-YOLOv5 models' performances on VisDrone2021 testset-dev for each category."  
...
[10] John Winn, and Andrew Zisserman. The pascal visual object classes (voc) challenge. International journal of computer vision, 88(2):303–338, 2010.
...
[12] Golnaz Ghiassi, Tsung-Yi Lin, and Quoc V Le. Nas-fpn: Learning scalable feature pyramid architecture for object detection. In Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition, pages 7036–7045, 2019.
...
[15] Jingjing Gu, Tao Su, Qihong Wang, Xiaojiang Du, and Mohsen Guizani. Multiple moving targets surveillance based on a cooperative network for multi-uav. IEEE Commun. Mag., 56(4):82–89, 2018.
...
\`\`\`
Sensing of Environment, 21:6:159–153, 2018.
