...
The text content from the image is "ADVERSARIAL FEATURE LEARNING".  
...
The text content from the image is "Trevor Darrell".  

philkr@utexas.edu
Department of Computer Science
University of Texas, Austin  

The text content from the image is "Philipp Krähenbühl".  
...
The GAN framework trains a generator, such that no discriminative model \( D : \Omega_X \mapsto [0,1] \) can distinguish samples of the data distribution from samples of the generative distribution. Both generator and discriminator are learned using the adversarial (minimax) objective min max \( V(D,G) \), where  
...
$$
V ( D , E , G ) : = \mathbb { E } _ { \mathbf { x } \sim p _ { \mathbf { X } } } \Big [ \underbrace { \mathbb { E } _ { \mathbf { z } \sim p _ { E } ( \cdot | \mathbf { x } ) } \left[ \log D ( \mathbf { x } , \mathbf { z } ) \right] } _ { \log D ( \mathbf { x } , E ( \mathbf { x } ) ) } + \mathbb { E } _ { \mathbf { z } \sim p _ { \mathbf { Z } } } \Big [ \underbrace { \mathbb { E } _ { \mathbf { x } \sim p _ { G } ( \cdot | \mathbf { z } ) } \left[ \log \left( 1 - D ( \mathbf { x } , \mathbf { z } ) \right) \right] } _ { \log ( 1 - D ( G ( \mathbf { z } ) , \mathbf { z } ) ) } \Big ] .
$$  

The text content from the image is:

"Definitions Let p_GZ(x, z) := p_G(x|z)p_Z(z) and p_EX(x, z) := p_E(z|x)p_X(x) be the joint distributions modeled by the generator and encoder respectively. Ω := Ω_X × Ω_Z is the joint latent and"  

$$
The formula in the image can be written in LaTeX as follows:

```latex
V(D, G) := \mathbb{E}_{\mathbf{x} \sim p_X} [\log D(\mathbf{x})] + 
\underbrace{\mathbb{E}_{\mathbf{x} \sim p_G} [\log (1 - D(\mathbf{x}))]}_{\mathbb{E}_{\mathbf{z} \sim p_Z} [\log (1 - D(G(\mathbf{z})))]}
```
...
"Proof. From Proposition 2, we have that \( C(E,G) = 2 \text{D}_{\text{JS}}(P_{EX} || P_{GZ}) - \log 4 \). The Jensen-Shannon divergence \( \text{D}_{\text{JS}}(P || Q) \geq 0 \) for any \( P \) and \( Q \), and \( \text{D}_{\text{JS}}(P || Q) = 0 \) if and only if \( P = Q \). Therefore, the global minimum of \( C(E,G) \) occurs if and only if \( P_{EX} = P_{GZ} \), and at this point the value is \( C(E,G) = -\log 4 \). Finally, \( P_{EX} = P_{GZ} \) implies that the optimal discriminator is chance: \( D_{EG}^* = \frac{\text{d}P_{EX}}{\text{d}(P_{EX} + P_{GZ})} = \frac{\text{d}P_{EX}}{2 \text{d}P_{EX}} = \frac{1}{2} \). □"  
...
"Proposition 2 The encoder and generator's objective for an optimal discriminator \( C(E, G) := \max_D V(D, E, G) = V(D_{EG}^*, E, G) \) can be rewritten in terms of the Jensen-Shannon divergence between measures \( P_{EX} \) and \( P_{GZ} \) as \( C(E, G) = 2D_{JS}(P_{EX} || P_{GZ}) - \log 4 \)."  
...
$$
```latex
P_{E\mathbf{X}}(R) := \int_{\Omega} p_{E\mathbf{X}}(\mathbf{x}, \mathbf{z}) 1_{[(\mathbf{x}, \mathbf{z}) \in R]} \, d(\mathbf{x}, \mathbf{z}) = \int_{\Omega_{\mathbf{X}}} p_{\mathbf{X}}(\mathbf{x}) \int_{\Omega_{\mathbf{Z}}} p_{E}(\mathbf{z}|\mathbf{x}) 1_{[(\mathbf{x}, \mathbf{z}) \in R]} \, d\mathbf{z} \, d\mathbf{x}

P_{G\mathbf{Z}}(R) := \int_{\Omega} p_{G\mathbf{Z}}(\mathbf{x}, \mathbf{z}) 1_{[(\mathbf{x}, \mathbf{z}) \in R]} \, d(\mathbf{x}, \mathbf{z}) = \int_{\Omega_{\mathbf{Z}}} p_{\mathbf{Z}}(\mathbf{z}) \int_{\Omega_{\mathbf{X}}} p_{G}(\mathbf{x}|\mathbf{z}) 1_{[(\mathbf{x}, \mathbf{z}) \in R]} \, d\mathbf{x} \, d\mathbf{z}
```
$$  
...
```latex
\begin{align*}
D_{\text{KL}}(P \parallel Q) &:= \mathbb{E}_{\mathbf{x} \sim P}[\log f_{PQ}(\mathbf{x})] \\
D_{\text{JS}}(P \parallel Q) &:= \frac{1}{2}\left(D_{\text{KL}}\left(P \parallel \frac{P+Q}{2}\right) + D_{\text{KL}}\left(Q \parallel \frac{P+Q}{2}\right)\right),
\end{align*}
```
$$  
...
This appears to be an incomplete sentence, likely part of a larger discussion or explanation about probability measures and regions.  
...
$$
```latex
P_{\mathbf{X}}(R_{\mathbf{X}}) := \int_{\Omega_{\mathbf{X}}} p_{\mathbf{X}}(\mathbf{x}) 1_{[\mathbf{x} \in R_{\mathbf{X}}]} \, d\mathbf{x}

P_{\mathbf{Z}}(R_{\mathbf{Z}}) := \int_{\Omega_{\mathbf{Z}}} p_{\mathbf{Z}}(\mathbf{z}) 1_{[\mathbf{z} \in R_{\mathbf{Z}}]} \, d\mathbf{z}
```
$$  
...
"as measures over regions \( R_X \subseteq \Omega_X \) and \( R_Z \subseteq \Omega_Z \). We refer to the set of features and data samples in the support of \( P_X \) and \( P_Z \) as \( \hat{\Omega}_X := \text{supp}(P_X) \) and \( \hat{\Omega}_Z := \text{supp}(P_Z) \) respectively. D_{KL} (P || Q) and D_{JS} (P || Q) respectively denote the Kullback-Leibler (KL) and Jensen-Shannon divergences between probability measures \( P \) and \( Q \). By definition,"  
...
Therefore, in order to fool a perfect discriminator at (x, z) (so that 0 < D^*_{EG}(x, z) < 1), E and G must satisfy both (a) and (b). In this case, we can substitute the equality E(x) = z required by (a) into the equality G(z) = x required by (b), and vice versa, giving the inversion properties x = G(E(x)) and z = E(G(z)).  
...
If only one of these properties is satisfied, a perfect discriminator can infer the source of (x,z) with certainty: if only (a) is satisfied, (x,z) must be an encoder pair (x,E(x)) and D^*_{EG}(x,z)=1; if only (b) is satisfied, (x,z) must be a generator pair (G(z),z) and D^*_{EG}(x,z)=0.  
...
Theorem 2 If E and G are an optimal encoder and generator, then E = G⁻¹ almost everywhere; that is, G(E(x)) = x for P_X-almost every x ∈ Ω_X, and E(G(z)) = z for P_Z-almost every z ∈ Ω_Z.  
...
$$
```latex
C(E, G) = \mathbb{E}_{\mathbf{x} \sim p_{\mathbf{X}}} \left[ \mathbf{1}_{[E(\mathbf{x}) \in \hat{\Omega}_{\mathbf{Z}} \land G(E(\mathbf{x})) = \mathbf{x}]} \log f_{EG}(\mathbf{x}, E(\mathbf{x})) \right] + 
\mathbb{E}_{\mathbf{z} \sim p_{\mathbf{Z}}} \left[ \mathbf{1}_{[G(\mathbf{z}) \in \hat{\Omega}_{\mathbf{X}} \land E(G(\mathbf{z})) = \mathbf{z}]} \log (1 - f_{EG}(G(\mathbf{z}), \mathbf{z})) \right]
```
$$  

The text content from the image is "3.4 LEARNING".  
...
$$
( b ) \, z \in { \hat { \Omega } } _ { Z } \; \land \; G ( z ) = x
$$  
...
(b) \mathbf{z} \in \hat{\Omega}_{\mathbf{z}} \land G(\mathbf{z}) = \mathbf{x}
```
$$  

The text content from the image is:

"and log (1 - f_{EG}) ∈ (-∞, 0) P_EX-almost and P_GZ-almost everywhere"  

Proof. Given in Appendix A.4.  

$$
( \mathrm { a } ) ~ \mathbf { x } \in \hat { \Omega } _ { \mathbf { x } } ~ \land ~ E ( \mathbf { x } ) = \mathbf { z }
$$  
...
"V(D, E, G) are estimated using mini-batches of n samples {x(i) \~ px}i=1^n and {z(i) \~ pz}i=1^n drawn independently for each update step."  

The discriminator \( D \) in a standard GAN takes data samples \( \mathbf{x} \sim p_{\mathbf{X}} \) as input, making its learned intermediate representations natural candidates as feature representations for related tasks.  
...
$$
```latex
\mathbb{E}_{\mathbf{x} \sim p_{\mathbf{X}}} \left[ \underbrace{\mathbb{E}_{\mathbf{z}' \sim p_E(\cdot|\mathbf{x})} \left[ \log D(g_{\mathbf{X}}(\mathbf{x}), \mathbf{z}') \right]}_{\text{log } D(g_{\mathbf{X}}(\mathbf{x}), E(\mathbf{x}))} \right] + \mathbb{E}_{\mathbf{z} \sim p_Z} \left[ \underbrace{\mathbb{E}_{\mathbf{x}' \sim p_G(\cdot|\mathbf{z})} \left[ \log (1 - D(\mathbf{x}', g_{\mathbf{Z}}(\mathbf{z}))) \right]}_{\text{log}(1-D(G(\mathbf{z}), g_{\mathbf{Z}}(\mathbf{z})))} \right].
```
$$  

3.5 GENERALIZED BIGAN  

The text content from the image is "4 EVALUATION".  

The text content from the image is "4.1 BASELINE METHODS".  
...
The text content from the image is "4.3 IMAGENET".  

The text content from the image is:

4.2 PERMUTATION-INARIANT MNIST  
...
the data, but often capture some interesting aspects. Here, each of \(D\), \(G\), and \(E\) is a convnet. In all experiments, the encoder \(E\) architecture follows AlexNet (Krizhevsky et al., 2012) through the fifth and last convolution layer (\(conv5\)). We also experiment with an AlexNet-based discriminator \(D\) as a baseline feature learning approach. We set the latent distribution \(p_{Z} = [U(-1, 1)]^{200}\) – a 200D continuous uniform distribution. Additionally, we experiment with higher resolution encoder input images – \(112 \times 112\) rather than the \(64 \times 64\) used elsewhere – using the generalization described in Section 3.5. See Appendix C.2 for more architectural and training details.  
...
In Figure 4 we present sample generations G(ż), as well as real data samples x and their BiGAN reconstructions G(E(x)). The reconstructions, while certainly imperfect, demonstrate empirically that  
...
The text content from the image is "4.4 DISCUSSION".  
...
The image contains a proposition stated as follows:

**Proposition 1:** For any \( E \) and \( G \), the optimal discriminator \( D_{EG}^* := \arg\max_D V(D, E, G) \) is the Radon-Nikodym derivative \( f_{EG} := \frac{\mathrm{d}P_{EX}}{\mathrm{d}(P_{EX} + P_{GZ})} : \Omega \mapsto [0, 1] \) of measure \( P_{EX} \) with respect to measure \( P_{EX} + P_{GZ} \).  

Let the probability measure \( P_{EG} := \frac{P_{EX} + P_{GZ}}{2} \) denote the average of measures \( P_{EX} \) and \( P_{GZ} \). Both \( P_{EX} \) and \( P_{GZ} \) are each absolutely continuous with respect to \( P_{EG} \). Hence the RN derivatives \( f_{EG} := \frac{\mathrm{d}P_{EX}}{\mathrm{d}(P_{EX} + P_{GZ})} = \frac{1}{2} \frac{\mathrm{d}P_{EX}}{\mathrm{d}P_{EG}} \) and \( f_{GE} := \frac{\mathrm{d}P_{GZ}}{\mathrm{d}(P_{EX} + P_{GZ})} = \frac{1}{2} \frac{\mathrm{d}P_{GZ}}{\mathrm{d}P_{EG}} \) exist and sum to 1:  

The text content from the image is:

"Proposition 2 The encoder and generator's objective for an optimal discriminator \( C(E, G) := \max_D V(D, E, G) = V(D_{EG}^*, E, G) \) can be rewritten in terms of the Jensen-Shannon divergence between measures \( P_{EX} \) and \( P_{GZ} \) as \( C(E, G) = 2 D_{JS}(P_{EX} || P_{GZ}) - \log 4 \)."  

The text content from the image is:

"Proof. For measures P and Q on space Ω, with P absolutely continuous with respect to Q, the RN derivative f_{PQ} := dP/dQ exists, and we have"  

$$
```latex
C(E,G) = \max_{D} V(D,E,G) = V(D^{*}_{EG},E,G)

= \mathbb{E}_{(\mathbf{x},\mathbf{z}) \sim P_{EX}} [\log D^{*}_{EG}(\mathbf{x},\mathbf{z})] + \mathbb{E}_{(\mathbf{x},\mathbf{z}) \sim P_{GZ}} [\log (1 - D^{*}_{EG}(\mathbf{x},\mathbf{z}))]

= \mathbb{E}_{(\mathbf{x},\mathbf{z}) \sim P_{EX}} [\log f_{EG}(\mathbf{x},\mathbf{z})] + \mathbb{E}_{(\mathbf{x},\mathbf{z}) \sim P_{GZ}} [\log f_{GE}(\mathbf{x},\mathbf{z})]

= \mathbb{E}_{(\mathbf{x},\mathbf{z}) \sim P_{EX}} [\log (2f_{EG}(\mathbf{x},\mathbf{z}))] + \mathbb{E}_{(\mathbf{x},\mathbf{z}) \sim P_{GZ}} [\log (2f_{GE}(\mathbf{x},\mathbf{z}))] - \log 4

= \text{D}_{\text{KL}} \left( P_{EX} || P_{EG} \right) + \text{D}_{\text{KL}} \left( P_{GZ} || P_{EG} \right) - \log 4

= \text{D}_{\text{KL}} \left( P_{EX} || \frac{P_{EX}+P_{GZ}}{2} \right) + \text{D}_{\text{KL}} \left( P_{GZ} || \frac{P_{EX}+P_{GZ}}{2} \right) - \log 4

= 2 \text{D}_{\text{JS}} \left( P_{EX} || P_{GZ} \right) - \log 4.
```
$$  
...
$$
```latex
V(D,E,G) = \mathbb{E}_{(\mathbf{x},\mathbf{z})\sim P_{EX}} [\log D(\mathbf{x},\mathbf{z})] + \mathbb{E}_{(\mathbf{x},\mathbf{z})\sim P_{GZ}} [\log (1-D(\mathbf{x},\mathbf{z}))]

= \mathbb{E}_{(\mathbf{x},\mathbf{z})\sim P_{EG}} [2f_{EG}(\mathbf{x},\mathbf{z}) \log D(\mathbf{x},\mathbf{z})] + \mathbb{E}_{(\mathbf{x},\mathbf{z})\sim P_{EG}} [2f_{GE}(\mathbf{x},\mathbf{z}) \log (1-D(\mathbf{x},\mathbf{z}))]

= 2 \mathbb{E}_{(\mathbf{x},\mathbf{z})\sim P_{EG}} [f_{EG}(\mathbf{x},\mathbf{z}) \log D(\mathbf{x},\mathbf{z}) + f_{GE}(\mathbf{x},\mathbf{z}) \log (1-D(\mathbf{x},\mathbf{z}))]

= 2 \mathbb{E}_{(\mathbf{x},\mathbf{z})\sim P_{EG}} [f_{EG}(\mathbf{x},\mathbf{z}) \log D(\mathbf{x},\mathbf{z}) + (1-f_{EG}(\mathbf{x},\mathbf{z})) \log (1-D(\mathbf{x},\mathbf{z}))].
```
$$  

$$
```latex
\mathbb{E}_{\mathbf{x} \sim P}[g(\mathbf{x})] = \int_{\Omega} g \, \mathrm{d}P = \int_{\Omega} g \frac{\mathrm{d}P}{\mathrm{d}Q} \, \mathrm{d}Q = \int_{\Omega} gf_{PQ} \, \mathrm{d}Q = \mathbb{E}_{\mathbf{x} \sim Q}[f_{PQ}(\mathbf{x})g(\mathbf{x})].
```
$$  

$$
f _ { E G } + f _ { G E } = \frac { \mathrm { d } P _ { E X } } { \mathrm { d } ( P _ { E X } + P _ { G Z } ) } + \frac { \mathrm { d } P _ { G Z } } { \mathrm { d } ( P _ { E X } + P _ { G Z } ) } = \frac { \mathrm { d } ( P _ { E X } + P _ { G Z } ) } { \mathrm { d } ( P _ { E X } + P _ { G Z } ) } = 1 .
$$  
...
\[ a \log y + (1 - a) \log(1 - y) = a \text{ for any } a \in [0, 1]. \]
Thus, \( D_{EG}^* = f_{EG} \).  
...
The image contains a mathematical statement and a proof. The statement is:

"Note that arg max_y {a log y + (1 - a) log(1 - y)} = a for any a ∈ [0, 1]. Thus, D^*_EG = f_EG."

The proof concludes with a square symbol "□", which typically signifies the end of a proof in mathematical texts.

This statement and proof are related to information theory or decision theory, specifically concerning the concept of the expected value of a random variable under different probabilities. The notation suggests that this is part of a larger discussion or proof involving the minimax theorem or a similar concept in decision-making under uncertainty.  
...
As shown in Proposition 2 (Section 3), the BiGAN objective is equivalent to the Jensen-Shannon divergence between \( P_{EX} \) and \( P_{GZ} \). We now go a step further and show that this Jensen-Shannon divergence is closely related to a standard autoencoder loss. Omitting the \( \frac{1}{2} \) scale factor, a KL divergence term of the Jensen-Shannon divergence is given as  

The text in the image reads:

"where we abbreviate as \( f \) the Radon-Nikodym derivative \( f_{EG} := \frac{\mathrm{d}P_{EX}}{\mathrm{d}(P_{EX} + P_{GZ})} \in [0,1] \) defined in Proposition 1 for most of this proof."  
...
$$
```latex
P_{EX}(R) = \int_{\Omega_X} p_X(\mathbf{x}) \int_{\Omega_Z} p_E(\mathbf{z}|\mathbf{x}) \mathbf{1}_{[(\mathbf{x}, \mathbf{z}) \in R]} \, d\mathbf{z} \, d\mathbf{x}
= \int_{\Omega_X} p_X(\mathbf{x}) \left( \int_{\Omega_Z} \delta(\mathbf{z} - E(\mathbf{x})) \mathbf{1}_{[(\mathbf{x}, \mathbf{z}) \in R]} \, d\mathbf{z} \right) \, d\mathbf{x}
= \int_{\Omega_X} p_X(\mathbf{x}) \mathbf{1}_{[(\mathbf{x}, E(\mathbf{x})) \in R]} \, d\mathbf{x}

P_{GZ}(R) = \int_{\Omega_Z} p_Z(\mathbf{z}) \int_{\Omega_X} p_G(\mathbf{x}|\mathbf{z}) \mathbf{1}_{[(\mathbf{x}, \mathbf{z}) \in R]} \, d\mathbf{x} \, d\mathbf{z}
= \int_{\Omega_Z} p_Z(\mathbf{z}) \left( \int_{\Omega_X} \delta(\mathbf{x} - G(\mathbf{z})) \mathbf{1}_{[(\mathbf{x}, \mathbf{z}) \in R]} \, d\mathbf{x} \right) \, d\mathbf{z}
= \int_{\Omega_Z} p_Z(\mathbf{z}) \mathbf{1}_{[(G(\mathbf{z}), \mathbf{z}) \in R]} \, d\mathbf{z}
```
$$  

The text content from the image is as follows:

"Proof. Let \( R_{\mathbf{X}}^{0} := \left\{\mathbf{x} \in \Omega_{\mathbf{X}} : \mathbf{x} \neq G(E(\mathbf{x}))\right\} \) be the region of \( \Omega_{\mathbf{X}} \) in which the inversion property \( \mathbf{x} = G(E(\mathbf{x})) \) does not hold. We will show that, for optimal \( E \) and \( G \), \( R_{\mathbf{X}}^{0} \) has measure zero under \( P_{\mathbf{X}} \) (i.e., \( P_{\mathbf{X}}(R_{\mathbf{X}}^{0}) = 0 \)) and therefore \( \mathbf{x} = G(E(\mathbf{x})) \) holds \( P_{\mathbf{X}} \)-almost everywhere."  

$$
```latex
\begin{align*}
\mathrm{D}_{\mathrm{KL}} \left( P_{EX} \parallel \frac{P_{EX} + P_{GZ}}{2} \right) &= \log 2 + \int_{\Omega} \log \frac{\mathrm{d}P_{EX}}{\mathrm{d}(P_{EX} + P_{GZ})} \, \mathrm{d}P_{EX} \\
&= \log 2 + \int_{\Omega} \log f \, \mathrm{d}P_{EX},
\end{align*}
```
$$  
...
$$
```latex
P_{\mathbf{X}}(R^{0}_{\mathbf{X}}) = \int_{\Omega_{\mathbf{X}}} p_{\mathbf{X}}(\mathbf{x}) 1_{[\mathbf{x} \in R^{0}_{\mathbf{X}}]} \, d\mathbf{x}
= \int_{\Omega_{\mathbf{X}}} p_{\mathbf{X}}(\mathbf{x}) 1_{[(\mathbf{x}, E(\mathbf{x})) \in R^{0}]} \, d\mathbf{x}
= P_{E\mathbf{X}}(R^{0})
= P_{G\mathbf{Z}}(R^{0})
= \int_{\Omega_{\mathbf{Z}}} p_{\mathbf{Z}}(\mathbf{z}) 1_{[(G(\mathbf{z}), \mathbf{z}) \in R^{0}]} \, d\mathbf{z}
= \int_{\Omega_{\mathbf{Z}}} p_{\mathbf{Z}}(\mathbf{z}) 1_{[\mathbf{z} = E(G(\mathbf{z})) \land G(\mathbf{z}) \in R^{0}_{\mathbf{X}}]} \, d\mathbf{z}
= \int_{\Omega_{\mathbf{Z}}} p_{\mathbf{Z}}(\mathbf{z}) \underbrace{1_{[\mathbf{z} = E(G(\mathbf{z})) \land G(\mathbf{z}) \neq G(E(G(\mathbf{z})))]}}_{=0 \text{ for any } \mathbf{z}, \text{ as } \mathbf{z} = E(G(\mathbf{z})) \implies G(\mathbf{z}) = G(E(G(\mathbf{z})))} \, d\mathbf{z}
= 0.
```
$$  

OF THEOREM 2 (OPTIMAL GENERATOR AND ENCODER ARE INVERSES  
...
We'll first show that in region \( R_S := \Omega \setminus \text{supp}(P_{G\mathbf{Z}}) \), we have \( f = 1 \) \( P_{EX} \)-almost everywhere. Let \( R^{f<1} := \{ (\mathbf{x}, \mathbf{z}) \in R_S : f(\mathbf{x}, \mathbf{z}) < 1 \} \) be the region of \( R_S \) in which \( f < 1 \). Let's assume that \( P_{EX}(R^{f<1}) > 0 \) has non-zero measure. Then, using the definition of the Radon-Nikodym derivative,  

Next we will show that \( f > 0 \) holds \( P_{EX} \)-almost everywhere, and hence \( F \) is always well defined and finite. We then show that \( F \) is equivalent to an autoencoder-like reconstruction loss function.  
...
We'll make use of the definitions of \( P_{EX} \) and \( PGZ \) for deterministic \( E \) and \( G \) found in Appendix A.3. The integral term of the KL divergence expression given in (6) over a particular region \( R \subseteq \Omega \) will be denoted by  
...
\`\`\`
where ε is a constant smaller than 1. But P_EX(R^f<1) < P_EX(R^f<1) is a contradiction; hence P_EX(R^f<1) = 0 and f = 1 P_EX-almost everywhere in R_S, implying log f = 0 P_EX-almost everywhere in R_S. Hence F(R_S) = 0. □
\`\`\`  

Theorem 3 The encoder and generator objective given an optimal discriminator \( C(E, G) := \max_D V(D, E, G) \) can be rewritten as an \( \ell_0 \) autoencoder loss function  

Proposition 3 ensures that log f is defined \(P_{EX}\)-almost everywhere, and \(F(R)\) is well-defined. Next we will show that \(F(R)\) mimics an autoencoder with \(\ell_0\) loss, meaning \(F\) is zero for any region in which \(G(E(\mathbf{x})) \neq \mathbf{x}\), and non-zero otherwise.  

Proof. Proposition 4 (F(Ω \ supp(P_GZ)) = 0) and F(Ω \ supp(P_EX)) = 0 imply that R^1 := supp(P_EX) ∩ supp(P_GZ) is the only region of Ω where F may be non-zero; hence F(Ω) = F(R^1).  

$$
```latex
F(R) := \int_R \log \frac{\mathrm{d}P_{\mathbf{EX}}}{\mathrm{d}(P_{\mathbf{EX}} + P_{G\mathbf{Z}})} \, \mathrm{d}P_{\mathbf{EX}} = \int_R \log f \, \mathrm{d}P_{\mathbf{EX}}.
```
$$  
...
$$
```latex
P_{\mathbf{EX}}(R^{f<1}) = \int_{R^{f<1}} f \, \mathrm{d}(P_{\mathbf{EX}} + P_{GZ}) = \int_{R^{f<1}} f \, \mathrm{d}P_{\mathbf{EX}} + \int_{R^{f<1}} f \, \mathrm{d}P_{GZ} \leq \varepsilon P_{\mathbf{EX}}(R^{f<1})
...
"Proposition 3 \( f > 0 \) \( P_{EX}\)-almost everywhere."  

The KL divergence \( F \) outside the support of \( P_{GZ} \) is zero: \( F(\Omega \setminus \text{supp}(P_{GZ})) = 0 \).  
...
$$
P _ { E X } ( R ^ { f = 1 } ) = \int _ { R ^ { f = 1 } } f \, \mathrm { d } ( P _ { E X } + P _ { G Z } ) = \int _ { R ^ { f = 1 } } 1 \, \mathrm { d } ( P _ { E X } + P _ { G Z } ) = P _ { E X } ( R ^ { f = 1 } ) + P _ { G Z } ( R ^ { f = 1 } ) ,
$$  

The text content from the image is:

"and log (1 - f_EG) ∈ (-∞, 0) P_EX-almost and P_GZ-almost everywhere"  
...
\[ P_{\text{EX}}(R^{f=1}) = \int_{R^{f=1}} f \, d(P_{\text{EX}} + P_{GZ}) = \int_{R^{f=1}} 1 \, d(P_{\text{EX}} + P_{GZ}) = P_{\text{EX}}(R^{f=1}) + P_{GZ}(R^{f=1}) \]

This equation appears to be expressing a probability measure \(P_{\text{EX}}\) over a set \(R^{f=1}\) where the function \(f\) is integrated with respect to the combined probability measure \(P_{\text{EX}} + P_{GZ}\). The integral of the constant function \(1\) over this measure results in the sum of the probabilities \(P_{\text{EX}}(R^{f=1})\) and \(P_{GZ}(R^{f=1})\).  
...
So a point (x, E(x)) is in R^1 if x ∈ Ω̂_X, E(x) ∈ Ω̂_Z, and G(E(x)) = x. (We can omit the x ∈ Ω̂_X condition from inside an expectation over P_X, as P_X-almost all x ∉ Ω̂_X have 0 probability.) Therefore,  

Finally, with Propositions 3 and 5, we have \( f \in (0,1) \) \( P_{EX} \)-almost everywhere in \( R^1 \), and therefore \( \log f \in (-\infty,0) \), taking a finite and strictly negative value \( P_{EX} \)-almost everywhere.  

In practice, θ_G and θ_E are updated by moving in the positive gradient direction of this inverse objective ∇_θ_E,θ_G Λ, rather than the negative gradient direction of the original objective.  

$$
```latex
C(E,G) = 2 \text{D}_{\text{JS}} \left( P_{EX} || P_{GZ} \right) - \log 4
...
\mathbb{E}_{\mathbf{z} \sim p_{\mathbf{z}}} \left[ \mathbf{1}_{\left[G(\mathbf{z}) \in \hat{\Omega}_{\mathbf{x}} \wedge E(G(\mathbf{z})) = \mathbf{z}\right]} \log (1 - f_{EG}(G(\mathbf{z}), \mathbf{z})) \right]
```
$$  

$$
```latex
\Lambda(D, G, E) = \mathbb{E}_{\mathbf{x} \sim p_{\mathbf{X}}} \left[ \underbrace{\mathbb{E}_{\mathbf{z} \sim p_E(\cdot|\mathbf{x})} \left[ \log \left( 1 - D(\mathbf{x}, \mathbf{z}) \right) \right]}_{\log(1-D(\mathbf{x}, E(\mathbf{x})))} + \underbrace{\mathbb{E}_{\mathbf{z} \sim p_Z} \left[ \mathbb{E}_{\mathbf{x} \sim p_G(\cdot|\mathbf{z})} \left[ \log D(\mathbf{x}, \mathbf{z}) \right] \right]}_{\log D(G(\mathbf{z}), \mathbf{z})} \right].
```
$$  
...
\begin{equation}
D_{KL}\left(P_{GZ} \parallel \frac{P_{EX} + P_{GZ}}{2}\right) - \log 2 = \mathbb{E}_{\mathbf{z} \sim p_{\mathbf{Z}}} \left[\mathbf{1}_{[G(\mathbf{z}) \in \hat{\Omega}_{X} \land E(G(\mathbf{z})) = \mathbf{z}]} \log f_{GE}(G(\mathbf{z}), \mathbf{z})\right]
= \mathbb{E}_{\mathbf{z} \sim p_{\mathbf{Z}}} \left[\mathbf{1}_{[G(\mathbf{z}) \in \hat{\Omega}_{X} \land E(G(\mathbf{z})) = \mathbf{z}]} \log (1 - f_{EG}(G(\mathbf{z}), \mathbf{z}))\right]
\end{equation}
...
```latex
\begin{align*}
\mathrm{D}_{\mathrm{KL}} \left( P_{E\mathbf{x}} \, || \, \frac{P_{E\mathbf{x}} + P_{G\mathbf{z}}}{2} \right) - \log 2 &= F(\Omega) = F(R^1) \\
...
&= \mathbb{E}_{\mathbf{x} \sim p_{\mathbf{x}}} \left[ \mathbf{1}_{[E(\mathbf{x}) \in \hat{\Omega}_{\mathbf{z}} \land G(E(\mathbf{x})) = \mathbf{x}]} \log f(\mathbf{x}, E(\mathbf{x})) \right].
\end{align*}
```
$$  
...
```latex
\begin{align*}
\text{supp}(P_{E\mathbf{x}}) &= \left\{(\mathbf{x}, E(\mathbf{x})) : \mathbf{x} \in \hat{\Omega}_{\mathbf{x}}\right\} \\
\text{supp}(P_{G\mathbf{z}}) &= \left\{(G(\mathbf{z}), \mathbf{z}) : \mathbf{z} \in \hat{\Omega}_{\mathbf{z}}\right\}
\end{align*}
```
$$  
...
The image contains a mathematical expression that defines a set \( R^1 \) as the intersection of the supports of two probability distributions, \( P_{E\mathbf{x}} \) and \( P_{G\mathbf{z}} \). The support of these distributions is defined by the conditions:

\[ R^1 := \text{supp}(P_{E\mathbf{x}}) \cap \text{supp}(P_{G\mathbf{z}}) = \{(\mathbf{x}, \mathbf{z}) : E(\mathbf{x}) = \mathbf{z} \land \mathbf{x} \in \hat{\Omega}_{\mathbf{x}} \land G(\mathbf{z}) = \mathbf{x} \land \mathbf{z} \in \hat{\Omega}_{\mathbf{z}}\} \]

Here:
- \( E(\mathbf{x}) = \mathbf{z} \) means that the function \( E \) applied to the vector \( \mathbf{x} \) results in the vector \( \mathbf{z} \).
- \( \mathbf{x} \in \hat{\Omega}_{\mathbf{x}} \) indicates that \( \mathbf{x} \) belongs to the set \( \hat{\Omega}_{\mathbf{x}} \).
- \( G(\mathbf{z}) = \mathbf{x} \) means that the function \( G \) applied to the vector \( \mathbf{z} \) results in the vector \( \mathbf{x} \).
- \( \mathbf{z} \in \hat{\Omega}_{\mathbf{z}} \) indicates that \( \mathbf{z} \) belongs to the set \( \hat{\Omega}_{\mathbf{z}} \).

This definition essentially describes the set of all pairs \( (\mathbf{x}, \mathbf{z}) \) where the functions \( E \) and \( G \) map each other's arguments correctly within their respective domains.  
...
\`\`\`
Optimization For unsupervised training of BiGANs and baseline methods, we use the Adam optimizer (Kingma & Ba, 2015) to compute parameter updates, following the hyperparameters (initial step size α = 2 × 10⁻⁴, momentum β₁ = 0.5 and β₂ = 0.999) used by Radford et al. (2016). The step size α is decayed exponentially to α = 2 × 10⁻⁶ starting halfway through training. The mini-batch size is 128. ℓ₂ weight decay of 2.5 × 10⁻⁵ is applied to all multiplicative weights in linear layers (but not to the learned bias β or scale γ parameters applied after batch normalization). Weights are initialized from a zero-mean normal distribution with a standard deviation of 0.02, with one notable exception: BiGAN discriminator weights that directly multiply z inputs to be added to spatial convolution outputs have initializations scaled by the convolution kernel size – e.g., for a 5 × 5 kernel, weights are initialized with a standard deviation of 0.5, 25 times the standard initialization.
\`\`\`  
...
"Preprocessing To produce a data sample x, we first sample an image from the database, and resize it proportionally such that its shorter edge has a length of 72 pixels. Then, a 64 × 64 crop is randomly selected from the resized image. The crop is flipped horizontally with probability 1/2. Finally, the crop is scaled to [−1, 1], giving the sample x."  
...
The text content from the image is "C.2 IMAGENET".  

The text content from the image is:

"C.1 PERMUTATION-INARIANT MNIST"  
